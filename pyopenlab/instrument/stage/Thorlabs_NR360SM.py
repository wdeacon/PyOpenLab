...
from pyopenlab.instrument.stage.apt_vcp_motor import APT_VCP_motor
...
class Thorlabs_NR360SM(APT_VCP_motor):
    ...
    def set_home_parameters(self, velocity=6, homeAngle=0.1, channel=1, debug=False):
        ...
        bs = [
            chanIdent, homeDirection["reverse"], limitSwitch["hardwareReverse"], homeVel,
            offsetDistance]
...
class Thorlabs_NR360SM_UI(QtWidgets.QWidget, UiTools):
    ...
    def closeEvent(self, event):
        if self.debug > 0 or DEBUG == True:
            print("Widget closed - cleaning up threads")
        self.stop_threads_flag.set()
        self.angle_update_thread.join()
        if self.debug > 0 or DEBUG == True:
            print("Widget closed - clean up DONE!")
        event.accept()
        return
    ...
    def set_move_type(self):
        ...
        if self.debug > 0 or DEBUG == True:
            print("Type changed!", self.move_type)
    ...
    def set_rotation_speed(self):
        try:
            ...
            if self.rotation_speed > 20.0:
                print(
                    "Thorlabs_NR360SM_UI.set_rotation_speed says: Rotating speed too high - wouldn't want to break the stage? - Not changing velocity"
                )
                return
            else:
