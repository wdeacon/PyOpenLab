...
class SpectrometerAligner(Instrument):
    ...
    Andor-feedback variant: couples a spectrometer to a 3-axis stage and offers
    several search strategies (circle, grid, focus sweep) that move the stage and
    refine its position by optimising a merit function built from the measured
    spectrum.
    """
    ...
    def __init__(self, spectrometer, stage):
        ...
        )  #reentrant lock, so that it doesn't matter that both optimise, and iterate_points (which it calls) acquire the lock
