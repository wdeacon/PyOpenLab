...
class Spectrometer(Instrument):
    ...
    metadata_property_names = ('model_name', 'serial_number', 'integration_time', 'reference',
                               ...
                               'variable_int_enabled', 'background_gradient', 'background_constant',
        ...
        if self.background is not None:
            if self.reference is not None:
                ...
                if self.variable_int_enabled == True:
                    new_spectrum = (
                        (spectrum - (self.background_constant +
                                     self.background_gradient * self.integration_time)) /
                        ((self.reference -
                          (self.background_constant + self.background_gradient * self.reference_int)
                ...
                if self.variable_int_enabled == True:
                    new_spectrum = spectrum - (self.background_constant +
                                               self.background_gradient * self.integration_time)
                else:
    ...
    def save_spectrum(self, spectrum=None, attrs={}, new_deque=False):
        ...
        #save data in the default place (see pyopenlab.instrument.Instrument)
    ...
    def time_series(self, num_spectra=None, delay=None, update_progress=lambda p: p):  # delay in ms
        ...
        update_progress(0)
        metadata = self.metadata
        extra_metadata = {'number of spectra': num_spectra, 'spectrum end-to-start delay': delay}
...
class Spectrometers(Instrument):
    ...
    def add_spectrometer(self, spectrometer):
        assert isinstance(spectrometer,
                          Spectrometer), 'spectrometer must be an instance of Spectrometer'
    ...
    def process_spectra(self, spectra):
        ...
        return self._pool.map(lambda s_spectrum: s_spectrum[0].process_spectrum(s_spectrum[1]),
                              pairs)
    ...
    def mask_spectra(self, spectra, threshold):
        return [
            spectrometer.mask_spectrum(spectrum, threshold)
            for (spectrometer, spectrum) in zip(self.spectrometers, spectra)]
    ...
    def save_spectra(self, spectra=None, attrs={}):
        ...
        g = self.create_data_group(
            self.filename, attrs=attrs)  # create a uniquely numbered group in the default place
...
class SpectrometerControlUI(QtWidgets.QWidget, UiTools):
    ...
    def __init__(self,
                 spectrometer,
                 ui_file=os.path.join(os.path.dirname(__file__), 'spectrometer_controls.ui'),
                 parent=None):
        ...
        register_for_property_changes(self.spectrometer, 'variable_int_enabled',
        ...
        self.id_string.setText('{0} {1}'.format(self.spectrometer.model_name,
                                                self.spectrometer.serial_number))
    ...
    def button_pressed(self, *args, **kwargs):
        ...
        if sender is self.read_background_button:
            ...
            if 'background' in self.spectrometer.config_file:
                self.spectrometer.background = self.spectrometer.config_file[
                    'background'][:]  #load the background
                if 'background_constant' in self.spectrometer.config_file:
                    self.spectrometer.background_constant = self.spectrometer.config_file[
                        'background_constant'][:]
                if 'background_gradient' in self.spectrometer.config_file:
                    self.spectrometer.background_gradient = self.spectrometer.config_file[
                        'background_gradient'][:]
                if 'background_int' in self.spectrometer.config_file:
                    self.spectrometer.background_int = self.spectrometer.config_file[
                        'background_int'][...]
            ...
            if 'reference' in self.spectrometer.config_file:
                ...
                if 'reference_int' in self.spectrometer.config_file:
                    self.spectrometer.reference_int = self.spectrometer.config_file[
                        'reference_int'][...]
                self.referenced.blockSignals(True)
                self.referenced.setCheckState(QtCore.Qt.Checked)
                self.referenced.blockSignals(False)
    ...
    def update_references(self, *args, **kwargs):
        ...
        try:
            ...
            self.spectrometer._logger.info('No refence/background saved in slot %s to load' %
                                           args[0])
    ...
    def update_time_series_params(self):
        ...
        self.time_total_lcdNumber.display(
            np.round(self.spectrometer.num_spectra *
                     (self.spectrometer.integration_time + self.spectrometer.delay) / 1000,
                     decimals=0))
...
class SpectrometerDisplayUI(QtWidgets.QWidget, UiTools):
    ...
    def __init__(self,
                 spectrometer,
                 ui_file=os.path.join(os.path.dirname(__file__), 'spectrometer_view.ui'),
                 parent=None):
        ...
        self.figure_widget = self.replace_widget(self.display_layout, self.figure_widget,
                                                 self.plotbox)
    ...
    def update_display(self, spectrum):
        ...
        if self.enable_threshold.checkState() == QtCore.Qt.Checked:
            ...
            if isinstance(self.spectrometer, Spectrometers):
                spectrum = [
                    spectrometer.mask_spectrum(s, threshold)
                    for (spectrometer, s) in zip(self.spectrometer.spectrometers, spectrum)]
            else:
        ...
        if not self.plots[0].getPlotItem().listDataItems():
            ...
            if isinstance(self.spectrometer, Spectrometers):
                ...
                self.plotdata.append(self.plots[0].plot(
                    x=wavelengths,
                    y=spectrum,
                    pen=(0, len(list(range(self.spectrometer.num_spectrometers))))))
        else:
            if isinstance(self.spectrometer, Spectrometers):
                for spectrometer_nom in range(self.spectrometer.num_spectrometers):
                    self.plotdata[spectrometer_nom].setData(x=wavelengths[spectrometer_nom],
                                                            y=spectrum[spectrometer_nom])
            else:
...
class SpectrometersUI(QtWidgets.QWidget):
    ...
    def __init__(self, spectrometers):
        assert isinstance(spectrometers,
                          Spectrometers), "instrument must be an instance of Spectrometers"
