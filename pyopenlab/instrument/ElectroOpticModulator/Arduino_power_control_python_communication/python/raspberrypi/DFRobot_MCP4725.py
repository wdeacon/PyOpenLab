...
FullSine7Bit = [
    ...
    2048, 1947, 1847, 1747, 1648, 1550, 1453, 1358, 1264, 1172, 1082, 995, 910, 828, 749, 672, 600,
    530, 465, 403, 345, 291, 242, 197, 156, 120, 88, 61, 39, 22, 10, 2, 0, 2, 10, 22, 39, 61, 88,
    120, 156, 197, 242, 291, 345, 403, 465, 530, 600, 672, 749, 828, 910, 995, 1082, 1172, 1264,
    1358, 1453, 1550, 1648, 1747, 1847, 1947]
...
class MCP4725():
    ...
    '''
        global addr_G
        global voltage_G
        bus.write_word_data(addr_G, MCP4725_Write_CMD | (MCP4725_NORMAL_MODE << 1),
                            int((vol / float(voltage_G)) * 255))
    ...
    '''
        global addr_G
        global voltage_G
        bus.write_word_data(addr_G, MCP4725_WriteEEPROM_CMD | (MCP4725_NORMAL_MODE << 1),
                            int((vol / float(voltage_G)) * 255))
    ...
    '''
        ...
        for i in range(0, (maxV - int(maxV / up_num) - 1), int(maxV / up_num)):
            ...
            if enterV > voltage_G:
                ...
                enterV = 0
            bus.write_word_data(addr_G, MCP4725_Write_CMD | (MCP4725_NORMAL_MODE << 1),
                                int((enterV / float(voltage_G)) * 255))
            endtime = datetime.datetime.now()
