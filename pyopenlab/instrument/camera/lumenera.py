...
#                delattr(self.__class__,pname)
        ...
        with self.acquisition_lock:
            for i in range(10):
                try:
                    ...
                    print(
                        "Attempt number {0} failed to capture a frame from the camera: {1}".format(
                            i, e))
    ...
    def convert_frame(self, frame_pointer, frame_size):
        ...
        converted_frame = self.cam.ConvertFrameToRgb24(f,
                                                       frame_pointer)  #actually convert the frame
    ...
    def get_metadata(self):
        ...
        ret['image_size'] = (frame.width // frame.binningX, frame.height // frame.binningY)
        if frame.flagsX:
