# -*- coding: utf-8 -*-
"""Driver for the Princeton Instruments Pixis CCD camera.

Communicates with the Picam DLL. Intended for use alongside an Acton
spectrometer for Raman measurements.
...
class Pixis(Camera):
    """Pixis CCD camera driver backed by the Picam DLL."""
    ...
    def SetTemperatureWithLock(self, temperature):
        ...
        while PicamSensorTemperatureStatus[status_code] != "PicamSensorTemperatureStatus_Locked":
            print("TemperatureStatus: {3}[{2}] (current: {0}, target:{1})".format(
                self.GetSensorTemperatureReading(), temperature, status_code,
