...
class uc480(QtWidgets.QMainWindow, UiTools):
    ...
    def display_image(self, image):
        ...
        if len(image.shape) == 3:  # colour camera
            ...
            for i in range(image.shape[2]):
                ...
                self.horizontal_profile.plot(x=list(range(image.shape[1])),
                                             y=image[horizontal_line_position, :, i],
                                             pen=pen)
                self.vertical_profile.plot(x=image[:, vertical_line_position, i],
                                           y=list(range(image.shape[0])),
                                           pen=pen)
        else:  # monochrome camera
            pen = pg.mkPen(color='w', width=5)
            self.horizontal_profile.plot(x=list(range(image.shape[1])),
                                         y=image[horizontal_line_position, :],
                                         pen=pen)
            self.vertical_profile.plot(x=image[:, vertical_line_position],
                                       y=list(range(image.shape[0])),
                                       pen=pen)
    ...
    def display_camera_parameters(self, camera_parameters):
        ...
        if self.set_whitebalance:
            self.CurrentAutoWhitebalanceLabel.setText(str(camera_parameters['auto_whitebalance']))
        if self.set_gamma:
    ...
    def get_camera_parameters(self):
        ...
        if self.set_whitebalance:
            camera_parameters['auto_whitebalance'] = self.camera.auto_whitebalance
        if self.set_gamma:
            camera_parameters['gamma'] = self.camera.gamma
    ...
    def save_image(self, dummy_variable=False, group_name='images'):
        ...
        if image_format == 'hdf5':
            ...
            self.file_path = QtWidgets.QFileDialog.getSaveFileName(
                self, 'Save image', self.file_path,
    ...
    def new_hdf5_file(self):
        ...
        if self.df:
            self.df.close()
        # open new datafile
        self.df = pyopenlab.current_datafile()
        # close the databrowser gui
        if self.df_gui:
            self.df_gui.close()
...
class LiveViewThread(QtCore.QThread):
    ...
    def live_view(self,
        ...
        if save:
            ...
            self.image_array = np.empty(self.array_dim,
                                        dtype='uint8')  # unit8 for minimum file size
