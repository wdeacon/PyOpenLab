...
from pyopenlab.experiment.gui import ExperimentWithProgressBar
...
from pyopenlab.utils.image_with_location import datum_pixel
from pyopenlab.utils.image_with_location import ensure_2d
from pyopenlab.utils.image_with_location import ensure_3d
from pyopenlab.utils.image_with_location import ImageWithLocation
from pyopenlab.utils.image_with_location import locate_feature_in_image
...
class CameraWithLocation(Instrument):
    ...
    disable_live_view = DumbNotifiedProperty(
        False)  # Whether to disable live view while calibrating/autofocusing/etc.
    ...
    def __init__(self, camera=None, stage=None):
        ...
        self.datum_pixel = np.array(
            shape[:2]) / 2.0  # Default to using the centre of the image as the datum point
...
#        self.use_previous_datum_location = False
    ...
    def move_to_feature(self,
        ...
        if not ignore_position:
            try:
                if ignore_z_pos:
                    ...
                    self.move(
                        feature.datum_location)  #initial move to where we recorded the feature was
            except:
        ...
        image = self.color_image()
        assert isinstance(
            image, ImageWithLocation), "CameraWithLocation should return an ImageWithLocation...?"
        ...
        for i in range(max_iterations):
            try:
                ...
                last_move = np.sqrt(np.sum(
                    (new_position - image.datum_location)**2))  # calculate the distance moved
    ...
    def autofocus(self,
        ...
        if method == "centre_of_mass":
            ...
            elif (number_of_maxima == 1) and not (indices_of_maxima[0] == 0 or
    ...
    def calibrate_xy(self, update_progress=lambda p: p, step=None, min_step=1e-5, max_step=1000):
        ...
        pixel_shifts = []
        images = []
        for i, p in enumerate([[-step, -step, 0], [-step, step, 0], [step, step, 0],
                               [step, -step, 0]]):
            #          print 'premove'
        ...
        location_shifts = np.array([
            ensure_2d(im.attrs['stage_position'] - starting_location) for im in images])
        pixel_shifts = np.array(pixel_shifts)
        print(np.shape(pixel_shifts), np.shape(location_shifts))
        A, res, rank, s = np.linalg.lstsq(
            pixel_shifts, location_shifts)  # we solve pixel_shifts*A = location_shifts
        ...
        if fractional_error > 0.02:  # Check it was a reasonably good fit
            print("Warning: the error fitting measured displacements was %.1f%%" %
                  (fractional_error * 100))
        self.log(
            "Calibrated the pixel-location matrix.\nResiduals were {}% of the shift.\nStage positions:\n{}\n"
            "Pixel shifts:\n{}\nResulting matrix:\n{}".format(fractional_error * 100,
                                                              location_shifts, pixel_shifts,
...
class AcquireGridOfImages(ExperimentWithProgressBar):
    ...
    def prepare_to_run(self,
        ...
        self.dest = self.cwl.create_data_group(
        ...
        try:
            for y_index in y_indices:
                for x_index in x_indices:
                    ...
                    self.cwl.move(
                        centre_image.pixel_to_location(np.array([x_index, y_index]) *
                                                       scan_step)[:2])
