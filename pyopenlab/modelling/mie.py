...
from pyopenlab.utils.refractive_index_db import RefractiveIndexInfoDatabase
...
def main3():
    ...
    for r in rs:
        ...
        for wavelength in wavelengths:
            ...
            print("N:", n_particle)
            x, m = make_rescaled_parameters(n_med=n_medium,
        ...
        ax1.plot(wavelengths / 1e-9,
                 Xs_sca,
                 label="Scattering efficiency $Q_{sca} = \sigma_{sca}/\pi r^2$")
        ax1.plot(wavelengths / 1e-9,
                 Xs_ext,
                 label="Extinction efficiency $Q_{ext} = \sigma_{ext}/\pi r^2$")
        ax1.plot(wavelengths / 1e-9,
                 Xs_abs,
                 label="Absorption efficiency $Q_{abs} = \sigma_{abs}/\pi r^2$")
...
def scattering_cross_section(radius, wavelength):
    ...
    x, m = make_rescaled_parameters(n_med=n_med,
                                    n_particle=n_particle,
                                    r=radius,
                                    wavelength=wavelength)
    output = calculate_scattering_cross_section(m=np.asarray([m]),
                                                x=np.asarray([x]),
                                                r=np.asarray([radius]),
                                                n_max=40)
    return output
...
def main4():
    ...
    ratio_low = scattering_cross_section(
        radius=120e-9, wavelength=450e-9) / scattering_cross_section(radius=100e-9,
                                                                     wavelength=450e-9)
    ratio_high = scattering_cross_section(
        radius=120e-9, wavelength=580e-9) / scattering_cross_section(radius=100e-9,
                                                                     wavelength=580e-9)
