...
from pyopenlab.instrument.camera.camera_with_location_car72 import CameraWithLocation
from pyopenlab.instrument.camera.lumenera import LumeneraCamera
from pyopenlab.instrument.light_sources.cube_laser import CubeLaser
...
def takePlBg(laserPower):
    ...
    time.sleep(1.0)
    gPlBg.create_dataset(name='Power %s' % (laserPower),
...
def df_pl_with_laser_scan(power_iter=1,
                          number_of_iter=1,
                          laser_power=3,
                          number_of_spec=2,
                          Increment=1,
    ...
    lastEventName = sorted(list(log.keys()),
                           key=lambda event: log[event].attrs['creation_timestamp'])[-1]
    ...
    for i in range(power_iter):
        for i in range(
                number_of_iter):  #while stop_event.is_set()==False and iterations<spectra_num:
