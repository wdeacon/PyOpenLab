...
class VirtualDimer:
    ...
    def initialize_constants(self):
        ...
        self.plasma_freq = 2 * np.pi * 2.99792458e8 / (self.plasma_wl * 1e-9
                                                       )  #radial frequency, Constant for Au
    ...
    def calc_theta(self):
        theta_degrees = 27.171 - 0.091802 * self.np_size + 0.00096972 * pow(
            self.np_size, 2) + 1.8442e-5 * pow(self.np_size, 3)
        self.theta = theta_degrees * (np.pi / 180)

    def calc_eta(self):
        self.eta = (pow(self.gap_ri, self.chi) / self.env_ri) * np.log(1 + (
            (self.np_size / 2) * pow(self.theta, 2) / (2 * self.gap_size)))
    ...
    def calc_cavity_wl(self):  # calculates energies of lowest mode
        ...
        en = wp / np.sqrt(
            self.eps_inf + self.facet_width * (self.gap_ri**2) /
            (self.gap_size * 4.2))  # alpha antinodes: in NPoMv2 {4.2,9.4,15} to fit better?

        self.cavity_wl = self.wl_to_ev(en)

    def calc_antenna_wl(
            self):  #calculates coupled mode for spherical dimer using Felix Benz's circuit model
    ...
    def calc_mixing(self):  # calculates coupled plasmon from cavity mode coupled to antenna mode
        ...
        eo = 0.5 * (e1 + e2) - np.sqrt(
            0.25 * (e2 - e1)**2 + 0.11**2)  # V=0.09 in NPoMv2 for 5 modes, increase to match
