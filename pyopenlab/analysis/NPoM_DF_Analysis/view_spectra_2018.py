...
from traitsui.api import Tabbed
    ...
    traits_view = View(
        VGroup(
            ...
            Item('figure',
                 ...
                 show_label=False,
                 label="Last Alignment",
                 springy=True),
        ),
        resizable=True,
        title="Overview Viewer",
    )
...
#        self._scan_number_changed()
    ...
    def refresh_data(self):
        ...
        try:
            g = self._hdf5file["ParticleScannerScan_%d/Particle_%d" %
        ...
        if self.use_reference:
            mask = [r < self.reference_threshold for r in (reference - background)
                    ]  #mask dodgy pixels
        ...
        self.replot(
            image[
                ...
                old_div(image.shape[1], 2) - 50:old_div(image.shape[1], 2) + 50,], zscan,
    ...
    def replot(self, image, zscan, wavelengths, spectrum):
        ...
        if fig is None:
            return  #don't plot to a non-existent figure...
        ...
        ax1.imshow(zscan,
                   extent=(wavelengths.min(), wavelengths.max(), -4, 4),
                   aspect="auto",
                   cmap="cubehelix")
...
def fit_gaussians_to_z_scan(z_scan, z=None, background=None, threshold=0.2, smoothing_width=1.5):
    ...
    if background is None:  #similarly, get a head-start on the background from metadata
        ...
        try:
            background = z_scan.attrs.get(
                'background')  #this should work for datasets in an HDF5 file

        except:
            z = np.zeros(
                z_scan.shape[1]
            )  #fall back to zero.  NB it will take a *really* long time to converge like this...

    Particle_array = np.array(
        z_scan,
        dtype=np.float) - background  #first prepare background-subtracted, thresholded arrays
    scan_s = scipy.ndimage.filters.gaussian_filter1d(Particle_array,
                                                     smoothing_width,
                                                     axis=0,
                                                     mode='nearest')
    scan_s[scan_s < 0.] = 0.  #smooth and remove negative points so that we don't get NaNs
    scan_t = old_div((scan_s - scan_s.min(axis=0)),
                     (scan_s.max(axis=0) - scan_s.min(axis=0))) - threshold
    scan_t[
        scan_t <
        0.] = 0.  #background-subtracted, thresholded version of Particle_array - used for finding the peak
    ...
    peak_z = old_div(np.sum(scan_t * z[:, np.newaxis], axis=0),
                     np.sum(scan_t,
                            axis=0))  #the z[np.newaxis,:] is "numpy broadcasting", google it!
    standard_deviation = np.sqrt(
        old_div(np.sum(scan_s * (z[:, np.newaxis] - peak_z)**2, axis=0), np.sum(scan_s, axis=0)))
    #Next, construct a Gaussian in Z with the right parameters and use linear least squares to fit offset/peak to the data
    gaussians = np.exp(
        old_div(-(z[:, np.newaxis] - peak_z[np.newaxis, :])**2, (2 * standard_deviation**2)))
...
def extract_all_spectra(datafile, outputfile):
    ...
    group_names = sorted(
        group_names, key=lambda group_name: len(datafile[group_name])
    )[::
      -1]  #Sort ParticleScannerScan_ groups in order of size, so that the largest dataset (usually the one of interest) is named "scan0" in the summary file.
    ...
    for n, group_name in enumerate(group_names):
        ...
        Particle_groups = [group for key, group in list(data_group.items()) if "Particle_" in key
                           ]  #filter out scan groups from overview images, etc.

        if len(Particle_groups) > 0:
            shape = (len(Particle_groups),
                     Particle_groups[0]['alinger.z_scan_0'].attrs.get('wavelengths').shape[0])
            ...
            for i in range(len(Particle_groups)):
                ...
                try:
                    spectra[i, :], peak_z[i, :], standard_deviation[i, :], fitted_background[
                        i, :], correlation_coefficient[i, :] = fit_gaussians_to_z_scan(
                            data_group['Particle_%d/alinger.z_scan_0' % i])
...
if __name__ == "__main__":
    ...
    for filename in os.listdir('.'):
        ...
        if re.match('\d\d\d\d-[01]\d-[0123]\d.h5',
                    filename):  #Finds first instance of file with name in "yyyy-mm-dd.h5" format
            break
