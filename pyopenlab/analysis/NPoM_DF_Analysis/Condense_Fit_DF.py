...
from pyopenlab.analysis.NPoM_DF_Analysis import Condense_DF_Spectra as cdf
from pyopenlab.analysis.NPoM_DF_Analysis import DF_Multipeakfit as mpf
...
def run(raiseExceptions=raiseExceptions,
        intensityRatios=intensityRatios,
        statsOnly=statsOnly,
        pl=pl,
        npSize=npSize,
        npomTypes=npomTypes,
        consolidateScans=consolidateScans,
    ...
    if statsOnly == True:
        outputFileName = mpf.findH5File(
            os.getcwd(), nameFormat='MultiPeakFitOutput',
            mostRecent=True)  #finds the most recent file with given name format
        mpf.doStats(outputFileName,
                    stacks=False,
                    pl=pl,
                    npomTypes=npomTypes,
                    intensityRatios=intensityRatios,
        ...
        if extractFirst == True:
            summaryFile = cdf.extractAllSpectra(
                os.getcwd(),
                ...
                customScan=customScan,
                consolidated=consolidateScans,
                avgZScans=avgZScans
            )  #condenses Z-stack (inc. background subtraction and referencing) for each particle and makes summary file

            if pl == True:
                summaryFile, pl = cdf.transferPlSpectra(
                    os.getcwd(), startWl=505, start=startSpec, finish=finishSpec
                )  #background subtracts each PL spectra and transfers them to the existing summary file
        ...
        if raiseExceptions == True:
            ...
            mpf.fitAllSpectra(os.getcwd(),
                              ...
                              npomTypes=npomTypes,
                              raiseExceptions=raiseExceptions,
                              raiseSpecExceptions=raiseExceptions,
                              intensityRatios=intensityRatios,
                              upperCutoff=upperCutoff,
                              lowerCutoff=lowerCutoff)
