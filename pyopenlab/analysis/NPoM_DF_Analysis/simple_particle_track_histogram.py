...
from pyopenlab.datafile import current
...
group_len = len(next(iter(scan.values())))  # number of datasets in each particle group
scan = {  # read the scan into a dictionary (but not memory)
    n: p for n, p in scan.items() if ((
...
for spec in stack:
    ...
    maxima = sorted(maxima, key=lambda i: -smooth[i])  # sort according to height
    if len(maxima):  # if there are any maxima (peaks)
        maximum = spec.wl[maxima[0]]  # take the highest
        if bounds.min() < maximum < bounds.max():  # if it's within the full bounds
            grouped[closest(grouped, maximum)].append(spec)  # add it to the right bin
            peaks.append(maximum)  # keep the value
#%%compute averages in each bin

averages = {
    group: Spectrum(np.mean(spectra, axis=0), wl) if spectra else Spectrum(np.zeros(len(wl)), wl)
