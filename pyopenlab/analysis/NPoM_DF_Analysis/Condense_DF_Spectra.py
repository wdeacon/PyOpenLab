...
def findH5File(rootDir, mostRecent=True, nameFormat='date'):
    ...
    if nameFormat == 'date':
        h5Files = sorted([
            i for i in os.listdir('.') if re.match('\d\d\d\d-[01]\d-[0123]\d', i[:10]) and
        ...
        h5Files = sorted([
            ...
            if i.startswith(nameFormat) and (i.endswith('.h5') or i.endswith('.hdf5'))],
                         key=lambda i: os.path.getmtime(i))
...
def truncateSpectrum(x, y, startWl=450, finishWl=900):
    ...
    if xTrunc.size <= 10 and x.size <= 100:
        ...
        xTrunc, yTrunc = np.transpose(
            np.array([[i, y[n]] for n, i in enumerate(x) if startWl < i < finishWl]))
...
def consoliData(rootDir):
    ...
    with h5py.File(inputFile, 'a') as ipf:
        ...
        allScans = sorted([
            groupName for groupName in ipf.keys() if groupName.startswith(gScanFormat) and '%s0' %
        ...
        finalScanNo = sorted(
            [groupName for groupName in ipf.keys() if groupName.startswith(gScanFormat)],
        ...
        for n, scanName in enumerate(allScans):
            ...
            if '%s0' % gParticleFormat in ipf[scanName].keys():
                ...
                existingParticles = sorted(
                    [i for i in consolidatedScan.keys() if i.startswith(gParticleFormat)],
                ...
                for particleN, groupName in enumerate(particleGroups):
                    ...
                    for dataName in gParticleOld.keys():
                        ...
                        try:
                            newDataset = gParticleNew.create_dataset(dataName,
                                                                     data=gParticleOld[dataName])
                        except:
                            print(type(gParticleOld[dataName]))
...
def extractAllSpectra(rootDir,
    ...
    with h5py.File(inputFile, 'a') as ipf:
        ...
        with h5py.File(outputFile, 'a') as opf:
            ...
            allScans = sorted(
                [groupName for groupName in list(ipf.keys()) if groupName.startswith(gScanFormat)],
            ...
            if fileType == 'post-2018':
                ...
                while dParticleFormat is None:
                    for dSetName in list(ipf[allScans[0]]['Particle_' + str(particleN)].keys()):
                        if dSetName.startswith('alinger.z_scan') or dSetName.startswith(
                                'zScan') or dSetName.startswith('z_scan_0'):
...
def approximateLaserBg(xPl, yPl, yDf, plRange=[540, 820], plot=False):
    xTrunc, yTrunc = truncateSpectrum(xPl, yPl, startWl=505,
    ...
    if plot == True:
        ...
        for compN, compName in enumerate(comps.keys()):
            ax1.plot(xTrunc,
                     comps[compName] + yJoinMin,
                     '--',
                     label=f'Exp {compN}',
                     color=plt.cm.Dark2(4 + compN))
...
def transferPlSpectra(rootDir,
        ...
        with h5py.File(outputFile, 'a') as opf:
            ...
            for n, scanName in enumerate(allScans):
                ...
                for nn, groupName in enumerate(particleGroups):
                    ...
                    particleGroup = scan[groupName]
                    plGroupNames = [
                        i for i in ['dark field with irradiation', 'PL Spectra']
                    ...
                    if int(old_div(100 * nn, len(particleGroups))) in nummers:
                        ...
                        print('%s%% (%s spectra) transferred in %s min %s sec' %
                              (nummers[0], nn, mins, secs))
                        nummers = nummers[1:]
                        ...
                        if 'wavelengths' not in list(plData.attrs.keys()):
                            try:
                                plData.attrs['wavelengths'] = scan[
                                    'Particle_0/%s' % dParticleFormat].attrs['wavelengths']
                    ...
                    attrNames = [
                        'creation_timestamp', 'integration_time', 'laser_power', 'model_name',
                        'serial_number', 'tec_temperature']
    ...
    return outputFile, True  #String of output file name for easy identification later
