    ...
    h5Files = sorted(
        [
            i for i in os.listdir() if forMatches(
                i, nameFormat, extension=['h5', 'hdf5'] if extension == 'h5' else extension)
...
def removeCosmicRays(x, y, reference=1, factor=15, chonk=1):
    ...
    while cosmicRay == True and iteration < 20:
        ...
        d1 *= np.sqrt(
            reference)  #de-references the spectrum to enhance cosmic ray detection in noisy regions

        d1 = abs(d1)  #takes magnitude of first derivative
        d1Med = np.median(
            d1)  #finds median gradient -> dy/dx should be larger than this for a cosmic ray
        ...
        if old_div(
            ...
            for i in np.linspace(0 - nSteps, nSteps,
                                 2 * nSteps + 1):  #for a window centred around the spike
...
def truncateSpectrum(x, y, startWl=450, endWl=900, xOnly=False, buff=0, **kwargs):
    ...
    if xTrunc.size <= 10 and x.size <= 100:  #sometimes fails for very short arrays; this extra bit works better in those cases
        ...
        xTrunc, yTrunc = np.transpose(
            np.array([[i, y[n]] for n, i in enumerate(x) if startWl < i < endWl]))
...
def retrieveData(directory,
    ...
    with h5py.File(summaryFile, 'a') as f:  # opens summary file
        ...
        summaryAttrs = {
            key: mainDataset.attrs[key] for key in list(mainDataset.attrs.keys())
        ...
        for n, spectrum in enumerate(spectra):
            ...
            try:
                newSpectrum = removeCosmicRays(
                    ...
                    reference=reference)  #attempts to remove cosmic rays from spectrum

                if False in np.where(
                        newSpectrum == newSpectrum[0], True,
                        False):  #if removeCosmicRays and removeNaNs have worked properly
...
def retrievePlData(directory, summaryNameFormat='summary', first=0, last=0):
    ...
    with h5py.File(summaryFile, 'a') as f:  #Opens summary file
        ...
        reference = f['particleScanSummaries/%s/spectra' %
                      gPlName].attrs['reference'][()]  #gets reference from DF spectra metadata
        ...
        dPlNames = sorted(list(gPl.keys()), key=lambda dPlName: int(dPlName.split(' ')[-1]))[
            first:last]  #creates list of PL spectrum names within specified bounds
        ...
        plSpectRaw = np.array([gPl[dPlName].attrs['Raw Spectrum'][()] for dPlName in dPlNames
                               ])  #collects all PL spectra of interest
        ...
        for n, plSpectrum in enumerate(plData):
            ...
            try:
                plSpectrum = removeCosmicRays(
                    xPl, plSpectrum,
                    reference=plSpectrum)  #attempts to remove cosmic rays from PL spectrum

                if False in np.where(
                        plSpectrum == plSpectrum[0], True,
                        False):  #if removeCosmicRays and removeNaNs have worked properly
...
def plotInitStack(x, yData, imgName='Initial Stack', closeFigures=True, vThresh=2e-4):
    ...
    yDataTrunc = np.array([old_div(spectrum, spectrum[transIndex]) for spectrum in yDataTrunc
...
def plotInitPlStack(xPl, plData, imgName='Initial PL Stack', closeFigures=True, vThresh=5e-5):
    ...
    yDataTrunc = np.array([
        truncateSpectrum(xPl, plSpectrum, startWl=580)[1] for plSpectrum in plData
    ])  # truncate to remove laser leak
    xStack = truncateSpectrum(xPl, plData[0], startWl=580)[0]  # x axis
    yDataTrunc = np.array([old_div(plSpectrum, plSpectrum[0]) for plSpectrum in yDataTrunc
...
def testIfDouble(x,
    ...
    if len(yMaxs) == 0:
        ...
        if len(maxsSortedY) > 1:
            ...
            if yMax2 - yTrough > (yMax - yTrough) / doublesThreshold and xMax2 > lowerLimit and abs(
                    xMax - xMax2) > 30:
                isDouble = True
...
def normToTrans(x, y, transNorm=1, troughNorm=0.61, transInit=533, startWl=450, plot=False):
    ...
    xTrunc, yTrunc = truncateSpectrum(x, y, startWl=startWl,
                                      finishWl=600)  #Truncate data from 450 to 600 nm
...
def analyseNpomSpectrum(x,
    ...
    isNpom2, isDouble, xMax, yMax = testIfDouble(xRaw,
                                                 yRaw,
    ...
    if isNpom == True:
        yRawNorm, initMinWl, transHeight, transWl = normToTrans(xRaw,
                                                                yRaw,
                                                                transNorm=1,
                                                                troughNorm=0.61,
        ...
        rawPeakFindMetadata, weirdGauss, cmGauss = findMainPeaks(xRaw,
                                                                 yRaw,
                                                                 fwhmFactor=1.1,
                                                                 plot=plot,
        ...
        normPeakFindMetadata, weirdGauss, cmGauss = findMainPeaks(x,
                                                                  yRawNorm,
                                                                  fwhmFactor=1.1,
                                                                  plot=plot,
                                                                  midpoint=peakFindMidpoint,
        ...
        normIntensityRatio = metadata['Coupled mode intensity (normalised)'] / metadata[
            'Transverse mode intensity (normalised)']
...
def calcNoise(y, ySmooth, windowSize=5):
    ...
    noise = y - ySmooth
    newNoise = np.concatenate(
        (noise[:old_div(windowSize, 2)][::-1], noise, noise[old_div(-windowSize, 2):][::-1]))
...
def findGausses(x,
    ...
    for n in range(1, 10):
        ...
        maxdices = [
            i for i in maxdices
            if x[i] > oldCenter and ySmooth[i] > 0 and ySmooth[i] > noise[i] * noiseThresh]
...
def analysePlSpectrum(x,
    ...
    if True not in np.where(plMetadata['Peak Heights'] > 0, True,
                            False):  # If spectrum only has negative peaks, there is no fluorescence
...
def plotAllStacks(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for groupName in npom_types:
            ...
            spectraNames = sorted(list(gSpectra.keys()),
            ...
            if sort == True:
                if sortingMethod in ['all', None]:
                    sortingMethods = [
                        'Coupled mode wavelength', 'Transverse mode wavelength',
                        'Weird peak wavelength', 'Coupled mode intensity (raw)',
                        'Transverse mode intensity (raw)', 'Weird peak intensity (raw)']
...
def plotHistogram(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        spectraNames = sorted([
            ...
            if gSpectra[i].attrs['Coupled mode wavelength'] != 'N/A' and
            cmLowLim < gSpectra[i].attrs['Coupled mode wavelength'] < endWl],
                              key=lambda i: int(i[9:]))

        x = gSpectra[spectraNames[0]].attrs['wavelengths']

        peakPositions = [
            gSpectra[i].attrs['Coupled mode wavelength'] for n, i in enumerate(spectraNames)]
        ...
        for n, binEdge in enumerate(bins[:-1]):
            binSpecNames = np.array([
                i for i in spectraNames
                if binEdge < gSpectra[i].attrs['Coupled mode wavelength'] < bins[n + 1] and
                gSpectra[i].attrs['Intensity ratio (normalised)'] < irThreshold and
                truncateSpectrum(x, gSpectra[i][()]).min() > -irThreshold])

            binnedSpectraList[binEdge] = binSpecNames

            if len(binSpecNames) > 0:
                avgSpec = old_div(np.sum(np.array([gSpectra[i][()] for i in binSpecNames]), 0),
                                  len(binSpecNames))
                avgSpecRaw = old_div(np.sum(np.array([gSpecRaw[i][()] for i in binSpecNames]), 0),
                                     len(binSpecNames))
        ...
        if plot == True:
            ...
            colors = [
                ...
                for n, yDataSum in enumerate(yDataPlot)][::-1]
...
def plotHistAndFit(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for binStart in binsSorted:
            binnedSpectraList[binStart] = sorted(binnedSpectraList[binStart],
        ...
        for n, binStart in enumerate(binsSorted):
            if len(binnedSpectraList[binStart]) > 0:
                ...
                if wLenned == False:
                    ...
                    gBin['Sum'].attrs['wavelengths'] = gSpectraBinned['Bin %s/Sum' %
                                                                      wlenN].attrs['wavelengths']

                for spectrumName in binnedSpectraList[binStart]:
                    gBin[spectrumName] = opf['NPoMs/%s/Raw/%s' % (npomType, spectrumName)]
                    gBin[spectrumName].attrs.update(opf['NPoMs/%s/Raw/%s' %
...
def plotDfPlHistogram(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for n, binEdge in enumerate(bins[:-1]):
            ...
            if len(binSpecNames) > 0:
                avgSpec = np.sum(np.array([gSpectra[i][()] for i in binSpecNames]),
                                 0) / len(binSpecNames)
                avgSpecRaw = np.sum(np.array([gSpecRaw[i][()] for i in binSpecNames]),
                                    0) / len(binSpecNames)
                avgPlSpec = np.sum(np.array([gPlSpectra[f'PL {i}'][()] for i in binSpecNames]),
                                   0) / len(binSpecNames)
...
def plotPlHistogram(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        spectraNames = sorted([
            i for i in gSpectra.keys() if gSpectra[i][()].max() < 10 and i[3:] in alignedSpecNames],
        ...
        peakVals = np.array([
            np.array(list(zip(gSpecRaw[i].attrs['Peak Centers'],
                              gSpecRaw[i].attrs['Peak Heights'])))
            for i in spectraNames
            if not any(gSpectra[i].attrs['Peak Heights'] > gSpectra[i][()][184:553].max() * 1.1)])
        ...
        yDataRawBinned = np.array(yDataRawBinned)
        yMax = max([
            truncateSpectrum(x, i, startWl=plRange[0], finishWl=plRange[1])[1].max()
        ...
        if plot == True:
            ...
            colors = [
                cmap(256 - n * (256 // len(yDataPlot))) for n, yDataSum in enumerate(yDataPlot)
            ][::-1]
            ...
            if peak.lower() != 'all':
                ...
                try:
                    plt.title('%s: %s\n%s peaks at:\n%s' %
                              (date, npomType, nPeaks, str([float('%.02f' % i) for i in resonance
                                                            ])[1:-1]))
                except:
        ...
        for n, binStart in enumerate(binsSorted):
            if len(binnedSpectraList[binStart]) > 0:
                ...
                if wLenned == False:
                    ...
                    gBin['Sum'].attrs['wavelengths'] = gSpectraBinned[f'Bin {wlenN:02}/Sum'].attrs[
                        'wavelengths']

                for spectrumName in binnedSpectraList[binStart]:
                    gBin[spectrumName] = opf['NPoMs/%s/PL Data/%s' % (npomType, spectrumName)]
                    gBin[spectrumName].attrs.update(opf['NPoMs/%s/PL Data/%s' %
                                                        (npomType, spectrumName)].attrs)
...
def plotAllHists(outputFileName,
    ...
    if npomTypes == 'all':
        npomTypes = [
            'All NPoMs', 'Non-Weird-Peakers', 'Weird Peakers', 'Ideal NPoMs', 'Doubles', 'Singles']
...
def plotHistComb2D(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        scatterKeys = np.concatenate(
            [['%s vs %s' % (dfKey, plKey) for dfKey in dfKeys] for plKey in plKeys]).ravel()
...
def reCheckCentering():
    ...
    with h5py.File(summaryFile, 'r') as ipf:
        return [
            specName for specName in ipf['Individual NPoM Spectra/scan0'].keys()
            if ipf['Individual NPoM Spectra/scan0'][specName].attrs['Properly centred?'] == True]


def plot_ir_grid(ax):
    ri_lines = {
        '1.0': np.array([[720., 690., 643., 622.], [4.81481, 3.7037, 2.31481, 1.38889]]),
    ...
    d_lines = {
        '0.5':
            np.array([[720., 754., 788., 824., 867.], [4.81481, 5.64815, 6.11111, 6.48148,
                                                       6.75926]]),
        '1':
            np.array([[690., 720., 751., 788., 820.], [3.7037, 4.44444, 5., 5.64815, 6.01852]]),
        '2':
            np.array([[643., 671., 701., 730., 757.], [2.31481, 2.87037, 3.33333, 3.61111,
                                                       3.88889]]),
        '3':
            np.array([[622., 645., 669., 694., 717.], [1.38889, 1.85185, 2.22222, 2.68519,
                                                       2.96296]])}
...
def plotIntensityRatios(outputFileName,
                        ...
                        plot=True,
                        corrPars=[
                            1.0763150264374398e-16, -4.955034954727432e-13, 9.679921256656744e-10,
                            -1.0400936324948906e-06, 0.0006638040249482491, -0.2516124765012756,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        spectraNames = [i for i in gSpectra.keys() if i not in misaligned_npoms
                        ]  # and gSpectra[i].attrs['Aligned?'] == True]
        ...
        if spec_range is not None:
            ...
            spectraNames = sorted([i for i in spectraNames if i in allowed_spectra],
                                  key=lambda i: int(i.split(' ')[-1]))
            #print(spectraNames)
        ...
        x = np.array([
            gSpectra[spectrumName].attrs['Coupled mode wavelength'] for spectrumName in spectraNames
        ])
        y = np.array([
        ...
        if plot == True:
            ...
            pars1 = gMod1.make_params(amplitude=frequencies.max() / 2,
                                      center=bins[:-1][frequencies.argmax()] * 0.66,
                                      sigma=1)

            gMod2 = GaussianModel(prefix='g2_')
            pars2 = gMod2.make_params(amplitude=frequencies.max() / 2,
                                      center=bins[:-1][frequencies.argmax()] * 1.33,
                                      sigma=1)
            ...
            IR_centres = np.array([[
                gOut.params[f'g{g_n}_{param}'].value
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for plotName in plotNames:
            for dataType in ['Raw', 'Normalised']:
                _, _, centroid, path = plotIntensityRatios(outputFileName,
                                                           plotName=plotName,
                                                           dataType=dataType,
                                                           closeFigures=closeFigures,
                                                           plot=plot)
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for n, spectrumName in enumerate(spectraNames):
            ...
            if intensityRatio != 'N/A':
                ...
                gSpecIrVis['Transverse mode height'].attrs['wavelengths'] = np.array([
                    wavelengths[0], wavelengths[1]])

                gSpecIrVis['Coupled mode height'] = cmHoriz
                gSpecIrVis['Coupled mode height'].attrs['wavelengths'] = np.array([
                    wavelengths[0], wavelengths[1]])
...
def calcGroupAttrAvgs(group):
    ...
    spectraNames = sorted(
        [spectrumName for spectrumName in list(group.keys()) if spectrumName != 'Sum'],
        key=lambda spectrumName: int(spectrumName[9:]))
    attrAvgs = {}
...
def calcAllPeakAverages(outputFileName,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        if npTypes == 'all':
            npTypes = [
                'All NPoMs', 'Non-Weird-Peakers', 'Weird Peakers', 'Ideal NPoMs', 'Doubles',
                'Singles']
                ...
                if histAvgs == True:
                    ...
                    if singleBin == False:
                        ...
                        binNames = [
                            ...
                            if histBins[binName].attrs['Bin start (nm)'] < peakPos <
...
def analyseRepresentative(outputFileName, peakFindMidpoint=680, npTypes='all'):
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for npType in npTypes:
            ...
            biggestBinName = binNames[np.array([len(histBins[binName]) for binName in binNames
                                                ]).argmax()]
            avgBinNames = [
                binName for binName in binNames if histBins[binName].attrs['Bin start (nm)'] <
                cmPeakPos < histBins[binName].attrs['Bin end (nm)']]
            ...
            for binName in binNames:
                ...
                try:
                    ...
                    y = dAvg[()]
                    avgMetadata = analyseNpomSpectrum(x,
                                                      y,
                                                      avg=True,
...
def sortSpectra(rootDir,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for n, spectrumName in enumerate(
                sorted(gAllRaw.keys(), key=lambda spectrumName: int(spectrumName.split(' ')[-1]))):
            try:
...
def fitAllSpectra(rootDir,
    ...
    with h5py.File(outputFileName, 'a') as opf:
        ...
        for n, spectrum in enumerate(yData):
            ...
            plMetadataKeys = [
                'Fit Error', 'Peak Heights', 'Peak FWHMs', 'Fit', 'Peak Centers', 'NPoM?']
            ...
            if nn == 0:
                ...
                gAllRaw[spectrumName].attrs['wavelengths'] = gAllRaw['Spectrum %s' %
                                                                     first].attrs['wavelengths']
            ...
            if pl == True:
                ...
                if nn == 0:
                    ...
                    gAllPl[plSpecName].attrs['wavelengths'] = gAllPl['PL Spectrum %s' %
                                                                     first].attrs['wavelengths']

            if raiseExceptions == True:
                specAttrs = analyseNpomSpectrum(x,
                                                spectrum,
                                                peakFindMidpoint=peakFindMidpoint,
                ...
                if pl == True:
                    plSpecAttrs = analysePlSpectrum(
                        xPl, plSpectrum,
                        raiseExceptions=raiseSpecExceptions)  #Main PL analysis function
                ...
                try:
                    specAttrs = analyseNpomSpectrum(
                        ...
                        cmLowLim=cmLowLim,
                        raiseExceptions=raiseSpecExceptions,
                        upperCutoff=upperCutoff)  #Main spectral analysis function
                    plMetadataKeys = [
                        'Fit Error', 'Peak Heights', 'Peak FWHMs', 'Fit', 'Peak Centers', 'NPoM?']
                    plSpecAttrs = {key: 'N/A' for key in plMetadataKeys}
                    ...
                    gFailed[spectrumName].attrs['Failure reason'] = gAllRaw[spectrumName].attrs[
                        'Failure reason']
                    gFailed[spectrumName].attrs['wavelengths'] = gAllRaw[spectrumName].attrs[
                        'wavelengths']
                    plMetadataKeys = [
                        'Fit Error', 'Peak Heights', 'Peak FWHMs', 'Fit', 'Peak Centers', 'NPoM?']
                    plSpecAttrs = {key: 'N/A' for key in plMetadataKeys}
                ...
                if pl == True:
                    ...
                    try:
                        ...
                        gFailedPl[plSpecName].attrs['Failure reason'] = gAllPl[plSpecName].attrs[
                            'Failure reason']
                        gFailedPl[plSpecName].attrs['wavelengths'] = gAllPl[plSpecName].attrs[
                            'wavelengths']
                        plMetadataKeys = [
                ...
                if pl == True:
                    ...
                    if type(gAllPl[plSpecName].attrs['Fit']) != str and abs(
                        ...
                        gAllNPoMsPlNorm[plSpecName] = old_div(
                            gAllPl[plSpecName][()], gAllPl[plSpecName].attrs['Fit'][()].max())
                        gAllNPoMsPlNorm[plSpecName].attrs.update(gAllPl[plSpecName].attrs)
                        gAllNPoMsPlNorm[plSpecName].attrs['Peak Heights'] = old_div(
                            gAllPl[plSpecName].attrs['Peak Heights'][()],
                            gAllPl[plSpecName].attrs['Fit'][()].max())

                    else:
                        gAllNPoMsPlNorm[plSpecName] = old_div(gAllPl[plSpecName][()],
