...
def fluorescence_BG_removal(xdata,
                            ydata,
                            test=True,
                            peakWidthRange=[3, 20],
    ...
    mins, mins_props = find_peaks(-ydata / np.max(ydata),
                                  width=peakWidthRange)  # mins are detected minima in the signal.
    ...
    BG1[current_ind:len(BG1)] = np.linspace(
        current, ydata[-1],
        len(BG1) - current_ind)  # BG1 is the line connecting from one detected minimum to the next.

    BG2 = [min(a, b) for a, b in zip(BG1, ydata)
           ]  # BG2 takes the minimum value between BG1 and the signal in every point
    ...
    if exclude_region is not None:  # used to exclude a part of the signal from the background calculation
        mins2 = [
            x for x in mins if ((xdata[x] < exclude_region[0]) or (xdata[x] > exclude_region[1]))
        ]  # finding the local minima which are in the exclusion region
        ...
        BG3 = [min(a, b) for a, b in zip(BG3, BG2)
               ]  # finally, BG3 is compared to BG2 and the minimum is taken.
    ...
    BG = baseline_als(BG3, lda=ALSparams[0], p=ALSparams[1],
                      niter=20)  # BG is the smooth background curve made by ALS method on BG3 line
