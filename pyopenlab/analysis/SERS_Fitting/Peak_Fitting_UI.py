...
def Select_Time_Range(Min, Max):
    ...
    while Continue is False:
        ...
        while Lower is None:
            Input = eval(
                input('Please Enter a Lower Spectrum Number (Type \'Cancel\' to cancel): '))
        ...
        while Upper is None:
            Input = eval(
                input('Please Enter a Upper Spectrum Number (Type \'Cancel\' to cancel): '))
            if str(Input).upper() == 'CANCEL':
                return None, None
            else:
                try:
    ...
    while End[0] is False:
        ...
        if Draw[0] is True:
            ...
            pl.imshow(Array,
                      ...
                      origin='lower',
                      cmap=colormap,
                      aspect=float(len(Array[0])) / len(Array),
                      vmax=Threshold[0])
...
def Input_Width():
    ...
    while Output is None:
        Input = eval(
            input(
                'Please Enter an Approximate Peak Width in Given X-Axis Units (Type \'Cancel\' to cancel): '
            ))
        if str(Input).upper() == 'CANCEL':
            return None
        else:
    ...
    for i in range(len(Array)):
        ...
        for j in Center_Guesses:
            ...
            while x_axis[Upper_element] < Upper_x:
                Upper_element += 1
            Heights.append(
                np.max(Array[i][Lower_element:Upper_element]) -
                np.min(Array[i][Lower_element:Upper_element]))
        ...
        Processes.append(
            Pool.apply_async(Fitting_Worker,
                             args=(Constant_plus_Lorentzians, x_axis, Array[i], Initial,
                                   (Bounds_Lower, Bounds_Upper), i)))
...
def Run(Array, x_axis, Threshold=None, colormap='inferno'):
    ...
    print('==========')
    print(
        "Please use your left mouse button to select peaks to track. Use your right mouse button to erase the last selection. Press Enter when done."
    )
    print('==========')
    ...
    print(
        "Please use your left mouse button to select section boundaries. Use your right mouse button to erase the last selection. Press Enter when done."
    )
    print('==========')
    ...
    for i in Sections:
        print('Fitting.....')
        Results.append(
            Run_Fitting(np.transpose(np.transpose(To_Fit)[int(i[0]):int(i[1])]),
                        x_axis[int(i[0]):int(i[1])], i[2], Width, Cores))
...
def View_Results(Array, x_axis, Start_Spectrum, End_Spectrum, Fit, Threshold=None):
    ...
    pl.imshow(To_Show,
              ...
              origin='lower',
              cmap='inferno',
              aspect=float(len(To_Show[0])) / len(To_Show),
              vmax=Threshold)
                ...
                while Pos1 < len(Sorting_Order):
                    ...
                    while Pos2 < len(Sorting_Order):
                        ...
                        if Sorting_Positions[Sorting_Order[Pos1]] is not None:
                            Change += abs(Current[Pos1] - Sorting_Positions[Sorting_Order[Pos1]]
                                          ) - abs(Current[Pos2] -
                                                  Sorting_Positions[Sorting_Order[Pos1]])
                        if Sorting_Positions[Sorting_Order[Pos2]] is not None:
                            Change += abs(Current[Pos2] - Sorting_Positions[Sorting_Order[Pos2]]
                                          ) - abs(Current[Pos1] -
                                                  Sorting_Positions[Sorting_Order[Pos2]])
