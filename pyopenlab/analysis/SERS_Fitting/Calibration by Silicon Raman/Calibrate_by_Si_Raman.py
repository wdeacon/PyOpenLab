...
def wavenumbers_from_si(wavelength_vector, si_stokes_wl, si_antistokes_wl):
    ...
    lda_laser1 = (-(si_stokes_wl - lda_det) + np.sqrt((si_stokes_wl - lda_det)**2 + 4 *
                                                      ((si_stokes_wl - lda_det) / si_wn))) / 2
    lda_laser3 = (-(si_antistokes_wl - lda_det) + np.sqrt((si_antistokes_wl - lda_det)**2 - 4 * (
        (si_antistokes_wl - lda_det) / si_wn))) / 2
...
def my_curve_fit(func, xdata, ydata, *initial_parameters):
    ...
    params1 = curve_fit(func, xdata=xdata, ydata=ydata, p0=init_params,
                        maxfev=100000)  #maxfev=10000
...
calc_params1, yfit1, Rsquared1 = my_curve_fit(fit_func, x_stokes1, y_stokes1,
                                              [1000, lda_stokes, 1, 300])
calc_params2, yfit2, Rsquared2 = my_curve_fit(fit_func, x_stokes2, y_stokes2,
                                              [300, lda_anti, 1, 300])
