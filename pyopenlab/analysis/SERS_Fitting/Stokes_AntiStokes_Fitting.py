...
def Fit(Shift,
        ...
        Smoothing_Width=30,
        Noise_Smoothing=2,
        Peak_Threshold_Window=5,
        Default_Peak_Width=10.,
        Minimum_Peak_Width=5.,
        Maximum_Peak_Width=30.,
        Iterations=2,
        Allowed_Peak_Fraction_In_Notch=0.1):

    Smooth = ndimf.gaussian_filter(Signal,
                                   old_div(Smoothing_Width, np.median(np.abs(np.diff(Shift)))))

    #--Gradient: Find notch filter--

    x = np.linspace(
        np.min(Shift), np.max(Shift),
        int(np.round(old_div((np.max(Shift) - np.min(Shift)), np.min(np.abs(np.diff(Shift)))))) + 1)
    ...
    x2 = np.linspace(
        0, np.min([np.max(Shift), -np.min(Shift)]),
        int(np.round(old_div((np.max(Shift) - np.min(Shift)), np.min(np.abs(np.diff(Shift)))))) + 1)
    ...
    Vector[-3] = np.max(
        Smooth[np.argmin(np.abs(Shift + 10)):np.argmin(np.abs(Shift - 10))]) - Constant
    ...
    while End is False:
        ...
        for i in range(len(Above_Std))[Skip:-Skip]:
            if np.sum(Above_Std[i - Skip:i - Skip +
                                Peak_Threshold_Window]) == Peak_Threshold_Window:
    ...
    for Iteration in range(Iterations):
            ...
            if np.sum(Possible_Peak) > 0:
                ...
                for i in np.array(list(range(len(Shift))))[Possible_Peak]:
                    Loss.append(
                        np.sum(
                            np.abs(
                                L(Shift, np.max([Sub[i], 0]), Shift[i], Default_Peak_Width) - Sub)))
                    Checked.append(i)

                Peaks_Vector += [
                    Sub[Checked[np.argmin(Loss)]], Shift[Checked[np.argmin(Loss)]],
                    Default_Peak_Width]
                Bounds += [(0, np.inf), (-np.inf, np.inf), (Minimum_Peak_Width, Maximum_Peak_Width)]
        ...
        for i in range(old_div(len(Peaks_Vector), 3)):
            Full_Bounds += [(0, np.inf), (-np.inf, np.inf),
                            (Minimum_Peak_Width, Maximum_Peak_Width)]
        ...
        while End is False:
            ...
            for i in range(old_div(len(Peaks_Vector), 3)):
                try:
                    Options.append(
                        spo.curve_fit(To_Fit,
                                      Shift,
                                      Sub,
                                      Peaks_Vector[:i * 3] + Peaks_Vector[i * 3 + 3:],
            ...
            if np.min(Loss) < Current_Loss:
                ...
                for i in range(old_div(len(Peaks_Vector), 3)):
                    Full_Bounds += [(0, np.inf), (-np.inf, np.inf),
                                    (Minimum_Peak_Width, Maximum_Peak_Width)]
                Output = spo.minimize(Full_Loss, Vector + Peaks_Vector,
                                      bounds=Full_Bounds).x.tolist()
                Vector = Output[:len(Vector)]
                Peaks_Vector = Output[len(Vector):]
