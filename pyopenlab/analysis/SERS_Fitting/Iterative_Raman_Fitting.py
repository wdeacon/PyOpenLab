...
def Add_New_Peak(x_axis, Signal, Current_Peaks, Width, Maximum_Width, Regions=50, Peak_Type='L'):
    ...
    for i in range(Regions):
        Bounds = [(0, np.inf), (i * Sections + Start, (i + 1) * Sections + Start),
                  (0, Maximum_Width)]
...
def Run(x_axis,
    ...
    if Peak_Type == 'L':
        ...
        Width = old_div(Wavelet_Estimate_Width(x_axis, Signal, Maximum_FWHM),
                        ((2. * np.log(2.))**0.5))
    ...
    while Regions <= len(Signal):
        ...
        if Initial_Fit is None:
            ...
            Result = copy.deepcopy(Results[-1]) + Add_New_Peak(
                x_axis, Signal, Results[-1], Width, Max_Width, Regions=Regions)
        ...
        if len(Loss_Results) > 1:
            if Loss_Results[-1] >= Loss_Results[-2]:  #Has loss gone up?
                ...
                while n < len(Results[-2]):
                    ...
                    Current.append([Results[-1][n + 1], Results[-1][n + 2]
                                    ])  #Collect peak positions and widths for last two iterations
                    n += 3
                ...
                for i in Old:
                    ...
                    Temp.append([Current[0][Arg],
                                 Current[1][Arg]])  #For each old peak, find the closest new peak
                ...
                while End is False:
                    ...
                    for i in range(
                            len(Old)
                    )[1:]:  #Check that swapping any adjacent sets of new peaks doesn't imporve the match between old and new peaks
                        Delta = abs(Old[i - 1][0] - Current[i - 1][0]) + abs(
                            Old[i][0] - Current[i][0]) - abs(Old[i - 1][0] -
                                                             Current[i][0]) - abs(Old[i][0] -
                                                                                  Current[i - 1][0])
                        if Delta > 0:
                            End = False
                            Current = Current[:i - 1] + [Current[i], Current[i - 1]
                                                         ] + Current[i + 1:]
                ...
                if True not in (Seperation >= Threshold
                                ).tolist():  #Check if all peaks have moved by less than 0.25 FWHM
                    Regions *= 5
    ...
    while Output is None and len(Results) > 0:
        ...
        while n < len(Results[-1]):
            ...
            n += 3
        try:
            Fits = spo.curve_fit(Final_Fitting_Function,
                                 x_axis,
                                 Signal, [0] + Results[-1],
                                 bounds=(BL, BU))
...
def Worker_Function(x_axis, Signal, Maximum_FWHM, Regions, Minimum_Width_Factor, Peak_Type,
                    Initial_Fit, Number):
    Fit = Run(x_axis,
              Signal,
              Maximum_FWHM,
    ...
    for i in range(len(Signals)):
        Processes.append(
            Pool.apply_async(Worker_Function,
                             args=(x_axis, Signals[i], Maximum_FWHM, Regions, Minimum_Width_Factor,
                                   Peak_Type, Initial_Peaks, i)))
