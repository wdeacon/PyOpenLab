...
The fullfit class is the main thing here - sample use:
    from pyopenlab.analysis.peaks_and_bg_fitting import fullfit
>>>ff = fullfit( 
                 spec,
                 shifts, 
                 lineshape = 'L',
...
class fullfit(object):
    ...
    def exponential(self, A, T, bg):
        ...
        return ((A * (np.exp((constants.hbar / constants.k) * omega / T) - 1))**-1) * interp(
            self.shifts, self.transmission)(self.shifts) + bg
    ...
    def exponential2(self, A, T, bg):
        ...
        '''
        omega = -cm_to_omega(self.shifts)
        return A * (((np.exp((constants.hbar / constants.k) * omega / T) - 1)**-1) + (np.exp(
            (constants.hbar / constants.k) * omega / 298.) - 1)**-1) * interp(
                self.shifts, self.transmission)(self.shifts) + bg
    ...
    def plot_result(self):
        ...
        plt.plot(self.shifts,
                 self.bg + self.multi_line(self.peaks) * self.transmission,
                 linestyle='--',
                 color='k')
    ...
    def add_new_peak(self):
        ...
        for i in range(int(self.regions)):
            bounds = [(0, np.inf), (i * sectionsize + Start, (i + 1) * sectionsize + Start),
                      (0, max(self.shifts) - min(self.shifts))]
            Centre = (i + np.random.rand()) * sectionsize + Start
            try:
                Height = max(
                    truncate(self.signal, self.shifts, i * sectionsize + Start,
                             (i + 1) * sectionsize + Start)[0]) - min(self.signal)
            except:
        ...
        while self.peak_added == False and i < (len(loss_results) - 1):
            ...
            if peak_candidate[
                    0] > self.noise_threshold and self.maxwidth > peak_candidate[2] > self.minwidth:
                if len(self.peaks) != 0:
                    dump, peak, residual = find_closest(peak_candidate[1],
                                                        np.transpose(self.peaks)[1])
    ...
    def initial_bg_poly(self):
        ...
        while len(self.bg_indices) < 3 * self.order:
            self.bg_indices = np.append(
                self.bg_indices,
                min(np.random.randint(0, high=len(self.spec), size=(10,)),
    ...
    def optimize_bg(self):  # takes bg_vals
        ...
        if self.asymm_peaks_stack == None:
            ...
            self.peaks_evaluated = self.asymm_multi_line(self.shifts,
                                                         self.asymm_peaks) * self.transmission
        ...
        if self.bg_type == 'poly':
            ...
            self.bg_p = curve_fit(self.bg_function,
                                  self.shifts,
                                  self.spec -
                                  self.multi_line(self.shifts, self.peaks) * self.transmission,
                                  p0=self.bg_p,
    ...
    def optimize_centre_and_width(self):
        ...
        for centre, width in zip(*centres_and_widths):
            centre_and_width_bounds.extend([(centre - width, centre + width),
                                            (width_bound)])  # height, position, width
        ...
        centres_and_widths = minimize(loss_centres_and_widths,
                                      np.ravel(centres_and_widths, 'F'),
                                      bounds=centre_and_width_bounds).x
            ...
            for index, peak in enumerate(self.peaks):
                try:
                    self.peaks[index][0] = max(
                        truncate(self.signal, self.shifts, peak[1] - peak[2] / 4.,
                                 peak[1] + peak[2] / 4.)[0])
                except:
    ...
    def optimize_peaks_and_bg(self):
        ...
        if self.bg_type == 'poly':
            ...
            if self.vary_const_bg == False:
                bounds = [(0, max(self.spec) * 10), (100, 1000), (min(self.spec * 0.7), 1e-9)]
            else:
                bounds = [(0, max(self.spec) * 10), (100, 1000),
    ...
    def optimize_asymm(self):
        ...
        if self.asymm_peaks is None:
            ...
            for peak in self.peaks:
                ...
                asymmbounds.extend([(peak[2] / 5., peak[2] * 5), (0.8, 1.2), (0.8, 1.2)])
            asymmbounds = np.array(asymmbounds)
        else:

            for asymm_peak in self.asymm_peaks:
                width_alpha_beta.extend([asymm_peak[2], asymm_peak[3], asymm_peak[4]])
                asymmbounds.extend([(old_div(asymm_peak[2], 5), asymm_peak[2] * 5), (0.9, 1.1),
                                    (0.9, 1.1)])
    ...
    def asymm_line(self, asymmpeak):
        ...
        alpha = np.true_divide(self.line(asymmpeak[0], asymmpeak[1], asymmpeak[2]),
                               asymmpeak[0])**asymmpeak[3]
        alpha = truncate(alpha, self.shifts, -np.inf, asymmpeak[1])[0]

        alpha *= asymmpeak[0]
        beta = np.true_divide(self.line(asymmpeak[0], asymmpeak[1], asymmpeak[2]),
                              asymmpeak[0])**asymmpeak[4]
    ...
    def _run(self,
        ...
        while self.regions <= len(self.spec):
            if verbose == True:
                print('Region fraction: ',
                      np.around(self.regions / float(len(self.spec)), decimals=2))
...
#            self.optimize_centre_and_width()
            ...
            if new_loss_score >= existing_loss_score:  # if fit has worsened, delete last peak
                ...
                for old_peak in Old:
                    ...
                    residual.append(
                        np.linalg.norm(np.array([1, old_pos]) - np.array([new_height, new_pos])))
                ...
                if type(comparison) == bool:  # happens if only 1 peak
                    ...
                    if all(comparison) == False:
                        self.regions *= 5
                        if verbose:
                            print("peaks haven't changed significantly; regions increased")
...
if __name__ == '__main__':
    from pyopenlab.analysis.example_data import SERS_and_shifts
