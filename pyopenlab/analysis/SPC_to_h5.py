...
class Raman_Spectrum(object):
    ...
    def __init__(self, filename, timestamp, metadata, laserWl, laserPower, absLaserPower,
                 integrationTime, accumulations, nScans, wavenumbers, ramanIntensities,
                 absRamanIntensities):
...
def extractRamanSpc(path, bg_path=False, combine_statics=False, encoding='utf-8'):
    ...
    powerConverter = {
        532: p532,
        633: p633,
        785: p785}  #Assigns each laser power dictionary to the appropriate wavelength.
    ...
    for n, spcFile in enumerate(spcFiles):
        ...
        for dictN, fDict in enumerate(fDicts):
            for k in list(fDict.keys()):
                ...
                if type(k) == bytes:
                    k = k.decode(
                    )  #spc module is written in python 2 and hasn't been updated yet; this ensures all strings are in the same (unicode) format
                if type(i) == bytes:
                    try:
                        i = i.decode()
                    except:
                ...
                if k.startswith(
                        ' '
                ):  #spc module imperfectly pulls data from some files and includes extra spaces in the dict keys
                    k = k[1:]
                if k.endswith(' '):
                    k = k[:-1]
        ...
        for unit in ['year', 'month', 'day', 'hour',
                     'minute']:  #the timestamp values are actually arbitrary, so this is obsolete
            if unit == 'year':
                zFill = 4
            else:
                zFill = 2
        ...
        if 'Laser_power' in list(fLogDict.keys()):
            ...
            laserPower = float(('').join([
                char for char in fLogDict['ND Transmission'].split(' ')[1]
        ...
        if laserPower in [0.0001, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0]:
            ...
            )  #Returns absolute laser power (in uW), given laser wavelength and % laser power.
        ...
        if absLaserPower != 'Undefined':
            ...
            absRamanIntensities = np.array([
                spectrum * 1000 / (absLaserPower * integrationTime * float(accumulations / 1000))
        ...
        spectra.append(
            Raman_Spectrum(filename, timestamp, metadata, laserWl, laserPower, absLaserPower,
                           integrationTime, accumulations, nScans, wavenumbers, ramanIntensities,
                           absRamanIntensities))
    ...
    with h5py.File(h5File, 'a') as f:
        ...
        if nameOnly == False:
            spectraNames = sorted(f['Spectra'].keys(),
                                  key=lambda spectrumName: int(spectrumName.split(':')[0][9:]))
        else:
            spectraNames = sorted(f['Spectra'].keys())
