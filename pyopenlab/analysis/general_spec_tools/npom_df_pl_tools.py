...
class NPoM_DF_Z_Scan(spt.Timescan):
    ...
    def condense_z_scan(self, threshold=0.01, plot=False, cosmic_ray_removal=True, **kwargs):
        ...
        max_indices = np.array([wl_scan.argmax() for wl_scan in Y_T
                                ])  #finds index of brightest z position for each wavelength
        max_indices_smooth = spt.butter_lowpass_filt_filt(max_indices, cutoff=900,
                                                          fs=80000)  #smooth
        '''
        ...
        centroid_indices = np.sum((Y_thresh * z_positions), axis=0) / np.sum(
            Y_thresh, axis=0)  #Find Z centroid position for each wavelength
        centroid_indices = spt.remove_nans(centroid_indices)

        assert np.count_nonzero(
            np.isnan(centroid_indices)
        ) == 0, 'All centroids are NaNs; try changing the threshold when calling condense_z_scan()'
...
class NPoM_DF_Spectrum(spt.Spectrum):
    ...
    def plot_df(self,
        ...
        if ax is None:
            if self.rc_params is not None:
                plt.rcParams.update(
                    self.rc_params)  #use rc params specified with object unless set to false
            if rc_params is not None:
    ...
    def test_if_npom(self,
        ...
        if np.sum(self.y - self.y.min()) < min_int_signal or (self.y_raw.min() < -0.1 and
                                                              self.x.min() < 500):
    ...
    def test_if_double(self, plot_is_double=False, **kwargs):
        ...
        maxs_sorted = sorted(
            [i for i in zip(x_maxs, y_maxs) if i[0] > self.cm_min_wl * 1.0345],
            key=lambda i: i[1])  # all maxima of smoothed spectrum within coupled mode region
        '''
        !!! Work in progress
        '''

    def multi_peak_fit(self,
                       peak_height_threshold=0.1,
                       peak_find_plot=False,
                       peak_fit_plot=False,
                       peak_shape='gaussian',
                       peak_find_height_frac=0.5,
                       **kwargs):
        #print(d2_plot)
        #print(self.x.min())

        approx_gausses = spt.approx_peak_gausses(self.x,
                                                 self.y_smooth,
                                                 height_frac=peak_find_height_frac,
                                                 plot=peak_find_plot,
                                                 threshold=peak_height_threshold,
                                                 peak_shape='gaussian',
                                                 **kwargs)
