...
from pyopenlab.analysis import spc_to_h5 as spc
...
def summarise_h5(data_dir,
                 h5_files,
                 summary_filename='SERS Summary.h5',
                 scan_format='ParticleScannerScan_',
                 scans_to_omit=[],
                 particle_format='Particle_',
                 sers_format='kinetic_SERS',
    ...
    for h5_file in h5_files:
        with h5py.File(h5_file, 'r') as F:
            scan_groups = [(scan_name, scan_group)
                           for scan_name, scan_group in F.items()
                           if scan_name.startswith(scan_format) and len(scan_group) > 3 and
                           scan_name not in scans_to_omit]

            all_particle_groups[h5_file] = []

            for scan_name, scan_group in scan_groups:
                particle_names = sorted(
                    [i for i in scan_group.keys() if i.startswith(particle_format)],
        ...
        with h5py.File(h5_file, 'r') as F:
            with h5py.File(summary_filename, 'a') as G:
                ...
                for n, particle_path in enumerate(all_particle_groups[h5_file], n + 1):
                    ...
                    sers_dsets = [
                        i[1] for i in g_particle_old.items() if i[0].startswith(sers_format)]
                    ...
                    particle_imgs = [
                        i for i in g_particle_old.items() if i[0].startswith(img_format)]

                    z_scans = [i for i in g_particle_old.items() if i[0].startswith(z_scan_format)]
...
def plot_all_timescans(h5_summary,
                       data_dir=None,
                       powder_spectrum=None,
                       agg_sers_spectrum=None,
                       dft_collection=None,
                       sers_format='kinetic_SERS',
                       save_figs=True,
                       x_scale=1,
    ...
    with h5py.File(h5_summary, 'r') as G:
        ...
        for n, (particle_name, particle_group) in enumerate(particle_groups):
            ...
            timescan = NPoM_SERS_Timescan(particle_group[sers_format],
                                          powder_spectrum=powder_spectrum,
                                          agg_sers_spectrum=agg_sers_spectrum,
                                          particle_name=particle_name,
                                          dft_collection=dft_collection,
                                          save_figs=save_figs,
                                          **kwargs)
...
class NPoM_SERS_Timescan(spt.Timescan):
    ...
    def __init__(self,
                 *args,
                 ...
                 agg_sers_spectrum=None,
                 dft_collection=None,
                 raman_excitation=633,
                 use_powder_xlim=True,
                 particle_name='Particle_',
                 find_dft=True,
                 **kwargs):
        ...
        if self.dft_collection is None and find_dft == True:
            self.dft_collection = drt.DFT_Raman_Collection(x_min=self.x_min,
                                                           x_max=self.x_max,
                                                           **kwargs)
    ...
    def plot_npom_sers_timescan(self,
                                particle_format='Particle_',
                                sers_format='kinetic_SERS',
                                show=True,
                                rc_params=timescan_params,
                                dft_polarisations=['yz'],
                                plot_averages=True,
                                x_min=None,
                                x_max=None,
                                x_scale=1,
        ...
        if self.agg_sers_spectrum is not None:
            ...
            ax_agg.plot(self.agg_sers_spectrum.x,
                        self.agg_sers_spectrum.y,
                        color=plt.cm.Dark2(3),
                        label='Agg SERS')
            ax_agg.set_yticks([])
            position = (0, 0)
        ...
        if self.powder_spectrum is not None:
            ...
            ax_powder.plot(self.powder_spectrum.x,
                           self.powder_spectrum.y,
                           'k',
                           label='Powder Raman')
        ...
        if save_figs == True:
            fig.savefig(f'Timescans/{self.particle_name}.png',
                        facecolor='white',
                        transparent=False,
                        bbox_inches='tight')
