...
def approx_peak_gausses(x,
    ...
    while True:
        ...
        for peak_index in maxima:
            ...
            y_diff_mins = detect_minima(abs(y_sub),
                                        upper_threshold=y.max() * 1e-2)  #find where y_sub crosses 0
        ...
        min_residual, chosen_gauss_params = sorted(zip(residuals, temp_gausses),
                                                   key=lambda r_g: r_g[0])[0]
...
def butter_lowpass_filt_filt(y, cutoff=1500, fs=60000, order=5, **kwargs):
    ...
    if len(
            y.shape
    ) == 2:  #if y is 2D array (ca. list of 1D spectra), each spectrum is smoothed individually through recursive calling
...
def calc_noise(y, y_smooth=None, window_size=5, **kwargs):
    ...
    noise = y - y_smooth
    new_noise = np.concatenate(
        (noise[:window_size / 2][::-1], noise, noise[-window_size / 2:][::-1]))
...
def find_d2_minima(x,
    ...
    d2_mins = d2_mins[
        d2[d2_mins]
        < -threshold]  # d2 minima that are negative and greater in magnitude than certain threshold
...
def remove_nans(data, noisy_data=False, **kwargs):
    ...
    if len(
            np.shape(y)
    ) > 1:  #if array dimensionality > 1, recursively performs remove_nans on each 1D sub array
    ...
    if noisy_data == True:
        ...
        '''
        y_smooth_interp = butter_lowpass_filt_filt(
            y_interp, **kwargs)  #include cutoff and fs values in kwargs, if needed
...
def truncate_spectrum(x, y, start_wl=450, end_wl=900, buffer=False):
    ...
    assert len(x) == len(
        y), f'x and y must be the same length. Currently len(x) = {len(x)}; len(y) = {len(y)}'
...
def remove_cosmic_rays(y, threshold=3, cutoff=1500, fs=60000, max_iterations=10, **kwargs):
    ...
    for i in range(max_iterations):
        ...
        if len(ray_indices) > 0:  # if there are any cosmic rays
            ...
            y_clean[ray_indices] = butter_lowpass_filt_filt(y_clean, cutoff, fs,
                                                            **kwargs)[ray_indices]
    ...
    def __init__(self,
        ...
        if len(args) == 1:
            if type(args[0]) in [list, np.ndarray]:  #if only y data provided
                ...
                assert type(
                    args[0]
                ) == h5py._hl.dataset.Dataset, f'{args[0].name} is an h5 group, not a dataset; please provide a dataset instead'
...
class Timescan(Spectrum):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.Y = self.y.copy()
        assert len(self.Y.shape
                   ) == 2, f'Input data should be a 2D array; current array shape is {self.Y.shape}'
    ...
    def determine_v_lims(self, min_std=0.5, max_std=4, ignore_vlims=False, **kwargs):
        ...
        bin_centres = np.linspace(np.average([bins[0], bins[1]]), np.average([bins[-2], bins[-1]]),
                                  len(frequencies))
