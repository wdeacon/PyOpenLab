...
def gaussian_from_area(x, area, centre, fwhm):
    ...
    height = area / (abs(fwhm) * np.sqrt(2 * np.pi)
                     )  # calculates height of curve based on area and fwhm

    return height * np.exp(-(((x - centre)**2) /
                             (2 * fwhm**2)))  # calculates curve based on height, centre and fwhm


def broaden_raman_activities(vib_freqs,
                             raman_activities,
                             scale_factor=0.9671,
                             fwhm=2,
                             x_min=0,
    ...
    y_cont = np.sum(
        np.array([gaussian_from_area(x_cont, a, c, fwhm) for c, a in zip(x, y)]), axis=0
    )  # transform each xy pair into gaussian centred at x coordinate, with area equal to y coordinate

    return x_cont, y_cont


def process_all_dft_raman(x_min=182.,
                          x_max=2616.,
                          polarisation=None,
                          data_dir=None,
                          extension='.log',
    ...
    for output_file in output_files:
        print(output_file)
        gaussian_output = Gaussian_Output(output_file,
                                          polarisation_vector=polarisation,
                                          x_min=x_min,
                                          x_max=x_max,
                                          **kwargs)
...
class Atom:
    ...
    def polTensorDer(self, d):  #d=0 dx, d=1, dy, d=2 dz
        ...
        tensor = [[der[d][0], der[d][1], der[d][3]], [der[d][1], der[d][2], der[d][4]],
                  [der[d][3], der[d][4], der[d][5]]]
...
class RamanCalculator():
    ...
    def calcRamanTensors(self):
        ...
        for k in range(0, len(self.job.vib_freq)):
            raman_tensors.append(np.matrix(np.zeros(
                (3, 3))))  # each vibration has a 3x3 Raman tensor (RT)
            ...
            for atom in self.job.molecule.atoms:
                if len(atom.vib_displ) > 0:  # Fixed atoms (-1) have no displacement
                    ...
                    alpha_dy = atom.polTensorDer(
                        1) * 0.279841  # units corrected from B^2 to A^2 (0.279841)
                    alpha_dz = atom.polTensorDer(2) * 0.279841  #

                    atom.pol_tensors.append(phi_x * alpha_dx + phi_y * alpha_dy + phi_z * alpha_dz)

                    raman_tensors[
                        k] += phi_x * alpha_dx + phi_y * alpha_dy + phi_z * alpha_dz  # adding up contributions to RT

            raman_tensors[k] /= np.sqrt(float(
                (self.job.red_mass[k])))  # normalising RT to reduced mass
    ...
    def calcRamanActivity(self):
        for raman_tensor in self.raman_tensors:
            ...
            g3 = np.square(raman_tensor[0, 1]) + np.square(raman_tensor[0, 2]) + np.square(
                raman_tensor[1, 2])
...
class Gaussian_Output:
    ...
    def __init__(self,
        ...
        gaussian_file = GaussianFile(
            self.filename)  #create instance of GaussianFile object from David's code
        ...
        if self.gaussian_file.raman == True:
            self.calculator = CalculatorMaster(
                gaussian_file)  #create instance of SERS calculator object
            ...
            if process_data == True:
                ...
                elif type(polarisation_vector == list) and len(polarisation_vector) > 0 and type(
                        polarisation_vector[0]) not in [int, float]:
    ...
    def extract_raman_activities(self, **kwargs):
        ...
        self.vib_freqs = np.array([i.split()[-3:] for i in F_lines if 'Frequencies' in i
                                   ]).flatten().astype(float)
        self.raman_activities = np.array([i.split()[-3:] for i in F_lines if 'Raman Activ' in i
                                          ]).flatten().astype(float)

        self.x_cont, self.y_cont = broaden_raman_activities(self.vib_freqs,
                                                            self.raman_activities,
    ...
    def extract_sers_activities(self, polarisation_vector='x', **kwargs):
        ...
        self.polarised_spectrum_dict[polarisation_vector] = broaden_raman_activities(
            freqs,
            activities,
            x_min=self.x_min,
            x_max=self.x_max,
...
class DFT_Raman_Collection:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        root_dir = os.getcwd()
        self.dft_dict = process_all_dft_raman(data_dir=dft_dir,
                                              filenames=dft_names,
                                              fwhm=fwhm,
    ...
    def get_polar_dict(self, polarisations={}):
        ...
        if len(
                polarisations
        ) == 0:  #extracts all gaussian jobs and all polarisations specified when initialising
            polar_dict = {
                gaussian_filename: list(gaussian_job.sers_activities_dict.keys())
                for gaussian_filename, gaussian_job in self.dft_dict.items()}

        elif type(polarisations) in (
                tuple, list
        ):  #extracts all gaussian jobs and all polarisations specified when running plot_dft
            polar_dict = {
                gaussian_filename: list(polarisations) for gaussian_filename in self.dft_dict.keys()}
            ...
            for m, polarisation in enumerate(polarisations):
                ...
                if text_loc is not False:
                    if text_loc == None:
                        ...
                        print(
                            'Text loc not recognised; please specify x-position (int or float), leave blank or set text_loc = False'
                        )
