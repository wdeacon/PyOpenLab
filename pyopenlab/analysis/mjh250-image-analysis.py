...
TAGS = [
    ...
    "Raman_Bias_Spectrum_int", "Raman_Bias_Spectrum_wl", "Raman_Laser_0Order_atBkgndLoc_int",
    "Raman_Laser_0Order_int", "Raman_Laser_Spectrum_atBkgndLoc_int",
    "Raman_Laser_Spectrum_atBkgndLoc_wl", "Raman_Laser_Spectrum_int", "Raman_Laser_Spectrum_wl",
    "Raman_White_Light_0Order_int", "Raman_White_Light_Bkgnd_0Order_int",
    "Raman_White_Light_Bkgnd_Spectrum_int", "Raman_White_Light_Bkgnd_Spectrum_wl",
    "Raman_White_Light_Spectrum_int", "Raman_White_Light_Spectrum_wl"]
TAG = TAGS[10]
...
def get_ellipse_contour(input_image):
    ...
    output_image, contours, hierarchy = cv2.findContours(output_image, cv2.RETR_TREE,
...
def make_vertex_mask(vertices, image_shape):
    ...
    for i in range(image_shape[0]):
        for j in range(image_shape[1]):
            if polygonPath.contains_points([[
                    j, i]]):  #ordering of indices is wrong way around to usual, but works!
                mask[i, j] = 1
    return mask
...
def apply_snake(image, contour):
    ...
    snake = skimage.segmentation.active_contour(image,
                                                init,
                                                alpha=0.005,
                                                beta=0.1,
                                                w_line=1e6,
...
def run_watershed(image, markers):
    return skimage.segmentation.watershed(image,
                                          markers,
                                          connectivity=1,
                                          offset=None,
                                          mask=None,
...
def process_image(image, figname):
    ...
    masked_image = masked_image[boxlims[1] + 1:min(80, boxlims[3]),
                                boxlims[0] + 1:min(80, boxlims[2])]
    ...
    images = [masked_image] + DoG + [
        prod, np_only, edge_segmented, fill_segmented, filled_valid, edge_valid]
    plot_multiscale(images,
                    figname,
                    valid="Filled: {0},Edge: {1}".format(
                        len(filled_valid_segments) > 0,
                        len(edge_valid_segments) > 0))
