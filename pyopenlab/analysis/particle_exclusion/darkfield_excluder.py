    ...
    def run(self, plot=False, overwrite=True):
        ...
        for name, group in tqdm(list(self.scan.items())):
            ...
            spec = temp_spec.split(
                450, 950).max(axis=0).remove_cosmic_ray()  # cleans and makes a single spectrum
            wl = spec.wl  # extracts wavelength vector
            smoothed_spec = ndimage.gaussian_filter1d(spec, sigma=self.sigma, axis=0)  # smooth data
            max_ind = np.where(
                smoothed_spec == np.max(smoothed_spec))[0]  # finds the global maximum
            ...
            if plot:
                ...
                plt.vlines(self.cutoff_wavelength,
                           np.min(spec),
                           np.max(spec),
                           colors=None,
                           linestyles='dashed')
