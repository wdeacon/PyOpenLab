...
from pyopenlab.analysis.particle_exclusion.utils import distance
from pyopenlab.analysis.particle_exclusion.utils import save_rejected
from pyopenlab.ui.ui_tools import QuickControlBox
from pyopenlab.utils.image_filter_box import Image_Filter_box
...
class ParticleProximityExcluder(QtWidgets.QWidget):
    ...
    def _construct(self):
        ...
        register_for_property_changes(self, 'exclusion_radius', self.update_image)  # prop to image
    ...
    def update_image(self, *value):
        ...
        if self.filter_box.current_filter_index == 1:
            centers, radii = self.filter_box.STBOC_with_size_filter(self.image,
                                                                    return_centers_and_radii=True)
            for c, r in zip(
                    centers, radii
            ):  # draw a white circle, if any red circle overlaps both particles should be discarded

                im = cv2.circle(im, tuple(c[::-1]), self.exclusion_radius + r, (255, 255, 255), 1)
    ...
    def run(self, path=None, overwrite=False):
        ...
        for name, group in tqdm(list(self.scan.items())):
            ...
            centers_radii = self.filter_box.STBOC_with_size_filter(im[()],
                                                                   return_centers_and_radii=True)

            if centers_radii is not None and len(centers_radii) > 1:
                center, radius = min(zip(*centers_radii), key=lambda c: distance(c[0], im_center))
                within = [
