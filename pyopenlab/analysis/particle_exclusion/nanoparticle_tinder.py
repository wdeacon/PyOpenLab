...
def reject(group, plot_function, cutoff=5000):
    ...
    group_len = len(next(iter(group.values())))  # number of datasets in each particle group
    scan = {  # read the scan into a dictionary (but not memory)
        n: p
        for n, p in group.items()
        if (n.startswith('Particle')  # has the right name
            and (n not in rejected)  # hasn't been rejected by another script
            )  # is full (nothing went wrong)
    }
    scan = {k: scan[k] for k in sorted(scan, key=lambda k: int(k.split('_')[-1]))}
