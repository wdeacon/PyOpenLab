...
import pyopenlab.analysis.NPoM_DF_Analysis.DF_Multipeakfit as mpf
...
class ExtinctionSpectrum:
    ...
    def fitAggPeaks(self, dimerWl=None, plot=False):
        ...
        initDimerFwhmEv = nmToEv(xFit[dimerIndex] - initDimerFwhm / 2) - nmToEv(xFit[dimerIndex] +
                                                                                initDimerFwhm / 2)
        ...
        if initChainIndex < dimerIndex:
            ...
            try:
                chainHalfMaxDex = dimerIndex + abs(chainInit[dimerIndex:initChainIndex] -
            ...
            try:
                ...
                initChainFwhmEv = nmToEv(initChainWl - initChainFwhm /
                                         2) - nmToEv(initChainWl + initChainFwhm / 2)
            except:
                if self.debug == True:
    ...
    def findDimerIndex(self, plot=False, returnAll=False):
        ...
        xLorz, yLorz = mpf.truncateSpectrum(xDimer,
                                            yDimer,
...
class AggExtDataset:
    ...
    def findStartPoint(self, plot=False, thresh=2, tInit=15):
        ...
        trapInts = np.asarray([
            np.trapz(y[startIndex:endIndex], x=x[startIndex:endIndex]) for y in yData])
        ...
        trapIntMins = np.array([
            i for i in mpf.detectMinima(trapInts) if t[i] < tInit and i > trapIntMaxs.max()])
        ...
        if len(trapIntMins) == 0:
            ...
            trapIntMins = np.array([
                i for i in mpf.detectMinima(d1)
                if t[i] < tInit and i > trapIntMaxs.max() and trapInts[i] < trapAvg * thresh])
    ...
    def fitSpectra(self, dSet, dimerPlot=False, debug=False):
        ...
        self.dimerCenters = np.array([
            specDict.get(n, ExtinctionSpectrum(None, None)).dimerCenter for n in nSpectra])
        self.chainCenters = np.array([
            specDict.get(n, ExtinctionSpectrum(None, None)).chainCenter for n in nSpectra])

        dimerWlAvg = np.nanmean(self.dimerCenters)
        dimerWlStd = np.nanstd(self.dimerCenters)
        self.dimerCenters = np.array([
            i if abs(dimerWlAvg - i) < dimerWlStd * 3 else np.nan for i in self.dimerCenters])

        chainWlAvg = np.nanmean(self.chainCenters[2:10])
        chainWlStd = np.nanstd(self.chainCenters[2:10])
        self.chainCenters[:10] = np.array([
            i if (abs(chainWlAvg - i) < chainWlStd * 2 and i > self.dimerCenters[n] and i > 0) else
            np.nan for n, i in enumerate(self.chainCenters[:10])])
        ...
        dSet.attrs['Dimer Heights'] = self.dimerHeights = np.array([
            specDict.get(n, ExtinctionSpectrum(None, None)).dimerHeight for n in nSpectra])
        dSet.attrs['Dimer Fwhms'] = self.dimerFwhms = np.array([
            specDict.get(n, ExtinctionSpectrum(None, None)).dimerFwhm for n in nSpectra])

        dSet.attrs['Chain Heights'] = self.chainHeights = np.array([
            specDict.get(n, ExtinctionSpectrum(None, None)).chainHeight for n in nSpectra])
        dSet.attrs['Chain Fwhms'] = self.chainFwhms = np.array([
            specDict.get(n, ExtinctionSpectrum(None, None)).chainFwhm for n in nSpectra])
...
class AggExtH5File():
    ...
    def getDsetNames(self, nDims=2):
        with h5py.File(self.filename, 'r') as f:
            ...
            dSetNames = [
                dSetName for dSetName in dSetNames if isinstance(f[dSetName], h5py.Dataset)]
    ...
    def findAunpSpectrum(self, reUse=True, extH5File=None):
        ...
        with h5py.File(h5File, 'a') as f:
            ...
            allSpecs = sorted([i for i in timeScans if 'AggInc' in f[i].attrs.keys()],
