...
if laser_type == "cw" or (laser_type == "pulsed" and laser_pulse_duration >= 1e-7):
    print("-----CW-----")
    linear_power_density = old_div(laser_output_power,
                                   beam_diameter)  #[units: W/cm - see Thorlabs LIDT tutorial]
...
if laser_type == "pulsed":
    ...
    print("[Pulsed] Beam Energy Density (Beam Profile Adjusted) [per Pulse] {} [J/m^2]".format(
        area_energy_density))
    ...
    if laser_pulse_duration >= 1e-9 and laser_pulse_duration < 1e-7:
        ...
        print(
            "[Pulsed] Damage Mechanism: Avalanche Ionization, WARNING - NO comparison for Thorlabs Damage Specs"
        )

    adjusted_lidt = area_energy_density * np.sqrt(old_div(
        laser_pulse_duration, lidt_pulse_duration)) * np.sqrt(
            laser_wavelength / float(lidt_wavelength))
