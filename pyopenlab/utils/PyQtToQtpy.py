...
def Convert_Pyqt_to_qtpy(path, avoid_files=None):
    ...
    file_locations = [
        os.path.join(dirpath, f)  #Create a list of file paths from the top level path
    ...
    for filename in file_locations:
        ...
        for line in fileinput.input(filename, inplace=True):
            if ('import ' in line) and (('QtCore' in line) or ('QtGui' in line) or ('uic' in line)):
                if '*' in line:
                    nplab_import_line = line.replace('PyQt4', 'pyopenlab.utils.gui')
                if ('QtWidgets' not in line) and ('QtGui' in line):
                    nplab_import_line = 'from pyopenlab.utils.gui import QtWidgets, '
                else:
                    nplab_import_line = 'from pyopenlab.utils.gui import '
                for attribute in dir(pyopenlab.utils.gui):
                    if attribute in line.replace(',', ' ').split('\n')[0].split(' '):
                        nplab_import_line = nplab_import_line + attribute + ', '
                if nplab_import_line == 'from pyopenlab.utils.gui import ':
                    sys.stdout.write(line)
                    continue
                else:
                    nplab_import_line = nplab_import_line[:-2] + '\n'
                    sys.stdout.write(
                        line.split('import')[0].split('from')[0] + nplab_import_line
                    )  # added line split on import and from to correct for white space for indented imports
                    continue
