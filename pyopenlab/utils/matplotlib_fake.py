...
def plot_skewed_image(image, corners, axes=None, *args, **kwargs):
    ...
    """
    shape = image.shape
    assert len(shape) == 2 or (len(shape) == 3 and (shape[2] == 3 or shape[2] == 4)
                               ), "the image argument must have shape (N*M) or (N*M*3) or (N*M*4)."

    if axes is None:
        axes = plt.gca()
    #First, we need to generate the coordinates of the vertices.  Start by
    #putting the corner coordinates in a 2x2x2 array
    assert np.array(corners).shape == (
        4, 2), "The 'corners' argument must be four two-element positions, i.e. have shape (4, 2)"
    ...
    coordinate_array = ndimage.zoom(corner_coordinate_array, (old_div(
        (shape[0] + 1), 2), old_div((shape[1] + 1), 2), 1),
                                    order=1)
    if len(shape) == 2:  #grayscale image
        mesh = ax.pcolormesh(coordinate_array[:, :, 0], coordinate_array[:, :, 1], image,
                             *args**kwargs)
    else:
        ...
        mesh = ax.pcolormesh(coordinate_array[:, :, 0], coordinate_array[:, :, 1], image[:, :, 0],
                             *args, **kwargs)
