...
def engineering_format(number,
                       ...
                       digits_of_precision=None,
                       print_errors=False):
    """Format a number into a string using SI prefixes."""
    assert not (
        significant_figures is not None and digits_of_precision is not None
    ), "You may not specify both the number of digits of precision and the number of significant digits."
    ...
    if number == 0:
        ...
        exponent = int(np.floor(old_div(np.log10(np.abs(number)),
                                        3))) * 3  #first power-of-three exponent smaller than number
        ...
        try:
            ...
            raise ValueError(
                "The number provided was too big (or too small) to convert to an SI prefix!")
    return "%.{0}g %s%s".format(digits_of_precision) % (mantissa, prefix, base_unit)


if __name__ == '__main__':
    print(
        engineering_format(2.0001e-6,
                           base_unit='m',
                           significant_figures=None,
                           digits_of_precision=6))
