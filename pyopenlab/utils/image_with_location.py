...
class ImageWithLocation(ArrayWithAttrs):
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            start = np.where(start == np.array(None), 0,
                             start)  # missing start points are equivalent to zero
            step = np.array([item[i].step for i in range(2)])
            step = np.where(step == np.array(None), 1,
                            step)  # missing step is equivalent to step==1
        except:
    ...
    def location_to_pixel(self, location, check_bounds=False, z_tolerance=np.inf):
        ...
        if check_bounds:
            ...
            assert np.abs(
                p[2]) < z_tolerance, "The location was too far away from the plane of the image"
        if len(location) == 2:
            return p[:2]
        else:
            return p[:3]
    ...
    def feature_at(self, centre_position, size=(100, 100), set_datum_to_centre=True):
        ...
        try:
            ...
            raise IndexError("Error: arguments of feature_at were invalid: {}, {}".format(
        ...
        thumb = self[pos[0] - size[0] // 2:pos[0] + size[0] // 2,
                     pos[1] - size[1] // 2:pos[1] + size[1] // 2, ...]
        if set_datum_to_centre:
            thumb.datum_pixel = (size[0] // 2, size[1] // 2
...
def locate_feature_in_image(image, feature, margin=0, restrict=False):
    ...
    assert image.shape[0] > feature.shape[0] and image.shape[1] > feature.shape[
        1], "Image must be larger than feature!"
    ...
    upper_margin = (image.shape[:2] - datum_pixel(image)) - (feature.shape[:2] -
                                                             datum_pixel(feature))
    assert np.all(
        np.array([lower_margin, upper_margin]) >= margin), "The feature image is too large."
    ...
    peak = ndimage.measurements.center_of_mass(
        corr)  # take the centroid (NB this is of grayscale values, not binary)
    pos = np.array(peak) + image_shift + datum_pixel(
        feature)  # return the position of the feature's datum point.
    return pos
