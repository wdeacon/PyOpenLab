...
#			print cls.extract_refractive_indices(dataset)
    ...
    def refractive_index_generator(cls, label):
        ...
        def generator(required_wavelength, scale="nm", debug=0):
            ...
            assert (scale == "nm"
                    )  #scale must be in nm - other values may be supported later if required
            ...
            if required_wavelength < min_wl:
                raise ValueError("Required wavelength: {0} below minimum in dataset: {1}".format(
                    required_wavelength, min_wl))

            elif required_wavelength > max_wl:
                raise ValueError("Required wavelength: {0} above maximum in dataset: {1}".format(
                    required_wavelength, max_wl))

            else:
                output_n = np.interp(required_wavelength * 1e6,
                                     xp=dataset["wavelength"],
                                     fp=dataset["n"])
                if debug > 0:
                    print("--- DEBUG Interpolation---")
                    print("Wavelen: {0}, Refractive_index: {1}".format(
                        required_wavelength, output_n))
                return output_n
