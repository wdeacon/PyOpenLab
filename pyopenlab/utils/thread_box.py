...
class ThreadBox3000(QuickControlBox):
    ...
    def add_controls(self, function):
        ...
        if (full_args.defaults != None and len(full_args.args) == len(full_args.defaults)):
            for arg, default in zip(full_args.args, full_args.defaults):
                if type(default) == int:
                    ...
                    if type(default) == np.ndarray:
                        ...
                        temp_txt = np.array2string(default).replace(
                            '   ', ',')  # danger - might need to check formatter
    ...
    def construct_payload(self):
        ...
        def payload(save_group=df._current_group):
            ...
            for variable in list(self.controls.keys()):
                ...
                if type(variable_control) == type(
                        QtWidgets.QLineEdit()) and variable_control.isReadOnly() == True:
            ...
            if self.controls['save_returned'].isChecked() == True:
                save_group.create_dataset(task, data=function_returns, attrs=input_variables)
