...
class Image_Filter_box(Instrument):
    ...
    def __init__(self,
        ...
        self.filter_options = [
            'None', 'STBOC_with_size_filter', 'strided_rescale', 'StrBiThresOpen', 'Grid']
    ...
    def STBOC_with_size_filter(self,
                               g,
                               return_centers=False,
                               return_centers_and_radii=False,
                               return_original_with_particles=False):
...
class Camera_filter_Control_ui(QuickControlBox):
    ...
    def __init__(self, filter_box):
        ...
        self.add_spinbox('area_size')
        self.add_spinbox('grid_pitch')
...
def strided_rescale(g, bin_fac=4):
    ...
    try:
        ...
        strided = np.uint8(
            (strided - np.min(strided)) * 254.0 / (np.max(strided) - np.min(strided)))
...
def StrBiThresOpen(g, bin_fac=4, threshold=40, bilat_size=3, bilat_height=40, morph_kernel_size=3):
    try:
        ...
        strided = cv2.bilateralFilter(np.uint8(strided), bilat_size, bilat_size,
                                      50)  # noise reduction
        #    strided[strided<threshold]=1
        #  strided = cv2.adaptiveThreshold(strided,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
        #       cv2.THRESH_BINARY,3,2)
        strided = cv2.adaptiveThreshold(strided, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY,
                                        101, -1 * threshold)
...
def STBOC_with_size_filter(g,
    ...
    for cnt in contours:
        ...
        if radius > max_size or radius < min_size:  # set the binary image to 0
            radius = int(radius) + 2
            strided[center[1] - radius:center[1] + radius,
                    center[0] - radius:center[0] + radius] = 0
        else:
...
def find_particles(self, img=None, border_pixels=50):
    ...
    self.threshold_image(self.denoise_image(cv2.cvtColor(
        frame, cv2.COLOR_RGB2GRAY)))[self.border_pixels:-self.border_pixels,
                                     self.border_pixels:-self.border_pixels]  #ignore the edges
    labels, nlabels = ndimage.measurements.label(img)
    return [
        np.array(p) + 15
        for p in ndimage.measurements.center_of_mass(img, labels, list(range(1, nlabels + 1)))
...
def Grid(g,
    ...
    x = [
        i for i in range(spacing, g_shape[0], spacing) for j in range(spacing, g_shape[1], spacing)]
    y = [
        j for i in range(spacing, g_shape[0], spacing) for j in range(spacing, g_shape[1], spacing)]
    centers = []
...
if __name__ == '__main__':
    ...
    cam_filter = Image_Filter_box()
    cam_filter.show_gui(blocking=False)
    #cam.filter_function = cam_filter.Grid#cam_filter.STBOC_with_size_filter
