...
def plot_wavelength_images(hsimg, axslice, roi, fig, ax, subplot_spec, amp=5):
    ...
    grid = ImageGrid(
        fig,
        subplot_spec,  # similar to subplot(111)
...
def plot_scan(hsimg, axslice, roi, amp=5):
    axial_wavelength, axial_spectrum = hsimg.integrate_spectra(xlims=(roi[0], roi[1]),
                                                               ylims=(roi[2], roi[3]),
                                                               axslice=axslice)
    transverse_wavelength, transverse_spectrum = hsimg.integrate_spectra(xlims=(roi[0], roi[1]),
                                                                         ylims=(roi[2], roi[3]),
                                                                         axslice=axslice,
                                                                         polarisation=2)
    ...
    gs1 = gridspec.GridSpecFromSubplotSpec(int(np.ceil(old_div(n_axes, 3))),
                                           3,
                                           gs0[0],
                                           hspace=0.05,
                                           wspace=0.05)
