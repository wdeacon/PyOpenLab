...
from nputils import get_nearest
from nputils import get_roi
...
class HyperspectralImage(object):
    ...
    def __init__(self, file_location, data_location='hyperspectral scans', scan_id=None):
    ...
    @property
    def wavelength(self):
        return self.scan['wavelength'][self._wavelength_roi]

    @property
    def energy(self):
        return old_div(1e9 * (old_div(h * c, self.wavelength)), e)
    ...
    def integrate_spectra(self,
        ...
        spectra = spectra[x_roi, y_roi, axslice, :] if spectra.ndim > 3 else spectra[x_roi,
                                                                                     y_roi, :]
    ...
    def get_line_spectra(self, axis, line, polarisation=1):
        wavelength, spectra = self.get_spectra(polarisation)
        if axis == 'x':
            line_spectra = np.array(spectra[:, line, :])
        elif axis == 'y':
            line_spectra = np.array(spectra[line, :, :])
        return line_spectra
...
if __name__ == '__main__':
    hsimg = HyperspectralImage('/Users/alansanders/Desktop/Data/2014/08. Aug/26/data.hdf5',
                               'hyperspectral scans', 2)
