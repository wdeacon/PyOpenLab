...
from pyopenlab.analysis.signal_alignment import correlation_align
from pyopenlab.analysis.smoothing import convex_smooth
from pyopenlab.analysis.wavelets import SUREShrink
...
def load_reference_data(reference_file, lower_wavelength, upper_wavelength, debug=0):
    ...
    if debug > 0:
        ...
        ax3.set_title(
            "Reference spectrum,\n Wavelength lower bound:{0}\n Wavelength upper bound:{1}".format(
...
def median_spectrum(data, debug=0):
    ...
    if debug > 0:
        ...
        ax.plot(wavelengths, [pixel_to_index(wl, lower_bound) for wl in wavelengths],
                label="Leftmost pixel index (0) (real pixel value: 10)")
        ax.plot(wavelengths, [pixel_to_index(wl, upper_bound) for wl in wavelengths],
                label="Rightmost pixel index (1014)")
        ax.legend()
...
def rescale_reference(xs, ys, max_size, N, debug=0):
    ...
    if debug > 0:
        ...
        ax.plot(xs)
        plt.plot(list(range(0, len(xs))), [offset + gradient * x for x in range(0, len(xs))],
                 label="least_squares fit")
        plt.legend()
...
def link_peaks(signal_peaks,
               reference_peaks,
               ignored_signal_indices,
               ignored_reference_indices,
               debug=0):
    ...
    if debug > 0:
        ...
        interp_diff = [
            interpolator_function(x)
            for x in range(int(np.min(signal_pos)), int(np.max(signal_pos)))]
...
def main(debug=0):
    ...
    ref_xs, ref_ys, gradient, offset = rescale_reference(xs=ref_xs,
                                                         ys=ref_ys,
    ...
    if debug > 0:
        ...
        wavelengths_reference = [
            index_to_wavelength(x, with_correction=False) for x in range(len(ref_ys))]
