...
def scan_fit(dataset, debug=0):
    ...
    for (center_wavelength, spectra, calibration_wavelengths) in dataset:
        ...
        if len(spectra) > 1:
            ...
            gradient, offset = single_position_fit(spectra,
    ...
    if debug > 0:
        ...
        ax1.set_title(
            "scan_fit: center wavelength vs wavelength offset (=$\lambda$ at pixel_index=0)")
        ax2.plot(center_wls, gradients, 'x-')
        ax1.set_xlabel("Center wavelength [nm]")
        ax1.set_ylabel("Pixel 0 wavelength (offset) [nm]")
        ax2.set_title(
            "scan_fit: center wavelength vs wavelength gradient (for determining wavelength for pixel_index > 0)"
        )
