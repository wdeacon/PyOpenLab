...
class Pacton(Instrument):
    ...
    def get_image(self, center_wavelength, roi=None, debug=0):
        ...
        if roi is not None:
            ...
            try:
                ...
                return self.pixis.get_roi(x_min=x_min,
                                          x_max=x_max,
                                          y_min=y_min,
                                          y_max=y_max,
                                          debug=debug)
    ...
    def pixel_to_wavelength(self, pixels, center_wavelength, intensity):
        ...
        if self.pixel_to_wl_sf is None:
            ...
            indices = np.logical_and(CALIBRATION_WAVELENGTHS >= min_wl, CALIBRATION_WAVELENGTHS
                                     < max_wl)
            ...
            upsampled_calibration_intensity = scipy.signal.resample(calibration_intensity,
                                                                    len(wavelengths),
                                                                    t=None,
                                                                    axis=0,
                                                                    window=None)
            upsampled_calibration_wavelengths = scipy.signal.resample(calibration_wavelengths,
                                                                      len(wavelengths),
                                                                      t=None,
                                                                      axis=0,
                                                                      window=None)
    ...
    def get_spectrum(self, center_wavelength, subtract_background=True, roi=None, debug=0):
        ...
        if subtract_background == True and self.pixel_response is None:
            raise ValueError(
                "Error getting spectrum with background subtraction - no background to subtract, please run: Pacton.get_pixel_response_calibration_spectrum "
            )
        if roi is not None:
            try:
                [x_min, x_max, y_min, y_max] = roi
                spectrum, pixel_offsets = self.pixis.get_spectrum(x_min=x_min,
                                                                  x_max=x_max,
                                                                  y_min=y_min,
                                                                  y_max=y_max)
    ...
    def calibrate(self, wavelengths, with_background_subtraction=False, plot=False, debug=0):
            ...
            for j in range(i + 1, len(spectra)):
                ...
                if plot == True:
                    p = axarr[1].plot(
                        xcs,
                        "-",
                        label="spectra: {0}nm vs {1}nm [offset: {2:.3g}, scale factor: {3:.3g}]".
                        format(wavelengths[i], wavelengths[j], peak, sf))
                    axarr[1].plot(peak, xcs[peak], "o", color=p[0].get_color())

                if debug > 0:
                    print(
                        "Scale factor [{0} nm] vs [{1} nm]:".format(wavelengths[i], wavelengths[j]),
                        sf)
        ...
        if debug > 0:
            print(
                "Final pixel to wavelength conversion  factor: {0:4g} +/- {1:4g} [nm/pixel]".format(
                    sf_mean, sf_std))
