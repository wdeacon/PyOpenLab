...
from pyopenlab.utils import gui_generator
...
import pyopenlab.experiment.dynamic_light_scattering as dls
from pyopenlab.instrument import Instrument
from pyopenlab.instrument.electronics.adlink9812 import Adlink9812
from pyopenlab.instrument.electronics.adlink9812 import Adlink9812UI
from pyopenlab.instrument.light_sources.fianium import Fianium
from pyopenlab.instrument.stage.Thorlabs_NR360SM import Thorlabs_NR360SM
    ...
    def __init__(self, instruments):
        ...
        if "sample_rotation_stage" in list(instruments.keys()):
            ...
            self.textboxes.update({
                "rotation_speed": self.sample_rotation_stage_ui.rotation_speed_textbox})
...
class DynamicLightScatteringUI(QtWidgets.QWidget, UiTools):
    ...
    def __init__(self, experiment, parent=None, debug=False, verbose=False):
        ...
        uic.loadUi(
            os.path.join(os.path.dirname(__file__), 'dynamic_light_scattering_experiment.ui'), self)
    ...
    def run_experiment(self):
        try:
            ...
            self.experiment.log("Error when running experiment - have you written the path?",
                                level="error")
...
config_loader = DynamicLightScattering(instruments={
    "adc": adc})  ##,"sample_rotation_stage":sample_rotation_stage})
...
gui = GuiGenerator(instrument_dict=instruments,
                   dock_settings_path=os.path.join(os.path.dirname(__file__), "experiment_ui.npy"),
                   scripts_path=None,
                   working_directory="~")
app.exec_()
