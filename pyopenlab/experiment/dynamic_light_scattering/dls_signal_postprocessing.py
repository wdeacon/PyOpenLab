...
def threshold(voltages, count_threshold=0.5):
    ...
    assert (len(nonzero_indices) == int(np.sum(
        np.absolute(pulses)))), "NONZERO LENGTH:{0},ABS SUM PULSES:{1}".format(
            len(nonzero_indices), np.sum(np.absolute(pulses)))
...
def binning(thresholded, index_bin_width):
    ...
    for i in range(outp_len):
        output[i] = np.sum(
            np.absolute(thresholded[i * index_bin_width:min(thresholded_len, (i + 1) *
                                                            index_bin_width)]))
