...
class GridScan(ScanningExperiment, TimedScan):
    ...
    def _update_axes(self, num_axes):
        ...
        current_size, current_step, current_init = (self.size.copy(), self.step.copy(),
                                                    self.init.copy())
        if self.num_axes > len(current_axes):
            self.axes = [''] * self.num_axes
            self.axes_names = [''] * self.num_axes
            self.size, self.step, self.init = (np.zeros(self.num_axes), np.zeros(self.num_axes),
    ...
    def run(self):
        ...
        self.acquisition_thread = threading.Thread(target=self.scan,
                                                   args=(self.axes, self.size, self.step,
                                                         self.init))
    ...
    def init_current_grid(self):
        ...
        axes, size, step, init = (self.axes[::-1], self.size[::-1], self.step[::-1],
                                  self.init[::-1])
...
class GridScanUI(QtWidgets.QWidget, UiTools):
    ...
    def __init__(self, grid_scanner):
        assert isinstance(grid_scanner,
        ...
        for widget, list, param in zip(
            [self.axes_view, self.axes_names_view, self.size_view, self.step_view, self.init_view],
            [
                self.grid_scanner.axes, self.grid_scanner.axes_names, self.grid_scanner.size,
