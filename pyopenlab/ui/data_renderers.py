    ...
    if isinstance(h5object, h5py.Group) and len(list(h5object.values())) > 100:
        ...
        for r in renderers:
            try:
                ...
                print("renderer {0} failed when checking suitability for {1} due to error: {2}".
                      format(r, h5object, e))
                pass  # renderers that cause exceptions shouldn't be used!
...
class AttrsRenderer(DataRenderer, QtWidgets.QWidget):
    ...
    def __init__(self, h5object, parent=None):
        ...
        uic.loadUi(os.path.join(os.path.join(os.path.dirname(__file__), 'hdf5_attrs_renderer.ui')),
                   self)
        ...
        if type(h5object) == list:
            item_info = QtWidgets.QTableWidgetItem(
                "Choose a single element to display its attributes!")
    ...
    def display_data(self):
        ...
        for h5object in list(self.h5object.values()):
            try:
                ...
                Ydata = np.array(h5object)
                Xdata = np.arange(len(Ydata))
            self.figureWidget.plot(x=Xdata,
                                   y=Ydata,
                                   name=h5object.name,
                                   pen=(icolour, len(self.h5object)))
            icolour = icolour + 1
    ...
    def display_data(self):
        ...
        for h5object in list(self.h5object.values()):
            ...
            self.figureWidget.plot(x=Xdata,
                                   ...
                                   pen=None,
                                   symbol='o',
                                   symbolPen=(icolour, len(self.h5object)),
                                   symbolBrush=(icolour, len(self.h5object)))
            icolour = icolour + 1
...
class MultiSpectrum2D(DataRenderer, QtWidgets.QWidget):
    ...
    def display_data(self):
        ...
        elif len(self.h5object.shape) == 1 and len(self.h5object.attrs['wavelengths']) < len(
                self.h5object) and len(self.h5object) % len(
                    self.h5object.attrs['wavelengths']) == 0:
        ...
        for h5object in data:
            ...
            if ((
                    variable_int == True
            ) and  #Check for variable integration time and that the background_int and reference_int are not none
                ((list(self.h5object.values())[i].attrs['background_int'] != list(
                    self.h5object.values())[i].attrs['integration_time'] and
                  (list(self.h5object.values())[i].attrs['background_int'] != None)) or
                 (list(self.h5object.values())[i].attrs['reference_int'] != list(
                     self.h5object.values())[i].attrs['integration_time'] and
                  (list(self.h5object.values())[i].attrs['reference_int'] != None)))):
                if list(self.h5object.values())[i].attrs['background_int'] != None:
                    if list(self.h5object.values())[i].attrs['reference_int'] != None:
                        data[i] = (
                            ((data[i] -
                              (list(self.h5object.values())[i].attrs['background_constant'] +
                               list(self.h5object.values())[i].attrs['background_gradient'] *
                               list(self.h5object.values())[i].attrs['integration_time'])) /
                             (((list(self.h5object.values())[i].attrs['reference'] -
                                (list(self.h5object.values())[i].attrs['background_constant'] +
                                 list(self.h5object.values())[i].attrs['background_gradient'] *
                                 list(self.h5object.values())[i].attrs['reference_int'])) *
                               list(self.h5object.values())[i].attrs['integration_time'] /
                               list(self.h5object.values())[i].attrs['reference_int']))))
                    else:
                        data[i] = data[i] - (
                ...
                if 'background' in list(list(self.h5object.values())[i].attrs.keys()):
                    if ListData == True:
                        if len(np.array(data[i])) == len(
                                np.array(list(self.h5object.values())[i].attrs['reference'])):
                            data[i] = data[i] - np.array(
                                list(self.h5object.values())[i].attrs['background'])
                    else:
                        if len(np.array(data)) == len(
                                np.array(list(self.h5object.values())[i].attrs['background'])):
                            data = data - np.array(
                                list(self.h5object.values())[i].attrs['background'])[:, np.newaxis]
                ...
                if 'reference' in list(list(self.h5object.values())[i].attrs.keys()):
                    if ListData == True:
                        ...
                        if len(np.array(data)) == len(
                            ...
                            data = (data / (
                                np.array(list(
                                    self.h5object.values())[i].attrs['reference'])[:, np.newaxis] -
                                np.array(list(
                                    self.h5object.values())[i].attrs['background'])[:, np.newaxis]))
...
#         print i,j,np.max(data) ,self.h5object.values()[i].attrs.keys()
        ...
        ConvertionM = ((list(self.h5object.values())[0].attrs['wavelengths'][-1] -
                        list(self.h5object.values())[0].attrs['wavelengths'][0]) /
                       len(list(self.h5object.values())[0].attrs['wavelengths']))
    ...
    def is_suitable(cls, h5object):
        ...
        if isinstance(h5object, dict) == False and isinstance(h5object, h5py.Group) == False:
            try:
                if len(h5object.shape) == 1 and (len(h5object) /
                                                 len(h5object.attrs['wavelengths'])) == 1:
                    return -1
            except:
                return -1
            h5object = {h5object.name: h5object}
            ...
            if 'wavelengths' in list(dataset.attrs.keys()):
                if len(dataset.shape) == 2:
                    if len(np.array(dataset)[:, 0]) < 100:
                        ...
                        return 1
                elif ((len(np.array(dataset)) / len(dataset.attrs['wavelengths']))) > 1 and (
                        len(np.array(dataset)) % len(dataset.attrs['wavelengths'])) == 0:
...
class JPEGRenderer(DataRenderer2or3DPG):
    ...
    @classmethod
    def is_suitable(cls, h5object):
        if h5object.attrs.get('compressed_image_format', None) in [
                'JPEG',
                'PNG',]:
            return 50
    ...
    def display_data(self):
        ...
        for h5object in list(self.h5object.values()):
            ...
            if ((
                ...
                ((h5object.attrs['background_int'] != h5object.attrs['integration_time'] and
                  (h5object.attrs['background_int'] != None)) or
                 (h5object.attrs['reference_int'] != h5object.attrs['integration_time'] and
                  (h5object.attrs['reference_int'] != None)))):
                Title = Title + " variable"
                if h5object.attrs['background_int'] != None:
                    if h5object.attrs['reference_int'] != None:
                        Data = (((Data - (h5object.attrs['background_constant'] +
                                          h5object.attrs['background_gradient'] *
                                          h5object.attrs['integration_time'])) /
                                 (((h5object.attrs['reference'] -
                                    (h5object.attrs['background_constant'] +
                                     h5object.attrs['background_gradient'] *
                                     h5object.attrs['reference_int'])) *
                                   h5object.attrs['integration_time'] /
                                   h5object.attrs['reference_int']))))
...
class HyperSpec(DataRenderer, QtWidgets.QWidget):
    ...
    def display_data(self):
        ...
        convertionfactors = [[YConvertionM, XConvertionM], [ZConvertionM, XConvertionM],
                             [ZConvertionM, YConvertionM]]
        ...
        for imgNom in range(len(Images)):
            Images[imgNom].setImage(Imagedata[imgNom],
                                    xvals=np.array(self.h5object.attrs['wavelengths']),
    ...
    def is_suitable(cls, h5object):
        if not isinstance(h5object, h5py.Dataset):
            return -1
        elif len(h5object.shape) == 4 and 'z' in list(h5object.attrs.keys()) and 'y' in list(
                h5object.attrs.keys()) and 'x' in list(h5object.attrs.keys()):
            return 30
...
class HyperSpec_Alan(DataRenderer, QtWidgets.QWidget):
    ...
    def display_data(self):
        ...
        for hyperspec_nom in range(1, num_hyperspec):
            ...
            Images.append(pg.ImageView(view=pg.PlotItem()))
            #Set image
            Images[hyperspec_nom - 1].setImage(data,
                                               xvals=np.array(self.h5object['wavelength2']),
                                               autoHistogramRange=True)
    ...
    #Image postion within a grid, (with need updating if using mroe than 4 spectrometers)
        ...
        for hyperspec_nom in range(num_hyperspec - 1):
            self.layout.addWidget(Images[hyperspec_nom], positions[hyperspec_nom][0],
                                  positions[hyperspec_nom][1])
...
class ScannedParticle(FigureRenderer):
    ...
    def display_data(self):
        ...
        ax1.imshow(zscan,
                   extent=(wavelengths.min(), wavelengths.max(), dz.min(), dz.max()),
                   aspect="auto",
                   cmap="cubehelix")
...
class PumpProbeShifted(DataRenderer, QtWidgets.QWidget):
    ...
    def display_data(self):
        if isinstance(self.h5object, dict) == False and isinstance(self.h5object,
                                                                   h5py.Group) == False:
            self.h5object = {self.h5object.name: self.h5object}
        icolour = 0
        Plots = []
        ...
        for h5object in list(self.h5object.values()):
            for axis in list(axes.keys()):
                ...
                for icolour in range(int(np.max(data[:, 7]) + 1)):
                    Plots[axis].plot(x=data[np.where(data[:, 7] == icolour)[0], 5],
                                     y=data[np.where(data[:, 7] == icolour)[0], axis],
                                     name=h5object.name,
                                     pen=(icolour, np.max(data[:, 7]) + 1))
                Plots[axis].setLabel('left', "dV/V")
                Plots[axis].setLabel('bottom', 'Time (ps)')
            icolour = icolour + 1
