        ...
        for control_type, c in list(auto_connectable_controls.items()):
            for control in self.findChildren(c['qt_type']):
                ...
                try:
                    ...
                    if verbose:
                        print("connected {0} '{1}' to {2}".format(
                            control_type, name, "UI" if obj is self else "target"))
            ...
            if isinstance(obj, QtWidgets.QComboBox):
                ...
                try:
                    ...
                    settings.setValue('/'.join([prefix, wdgt_name]),
                                      value)  # save combobox selection to registry
                except Exception as e:
            ...
            if isinstance(obj, QtWidgets.QComboBox):
                ...
                try:
                    ...
                    if value == "":
                        continue
                    index = obj.findText(
                        value)  # get the corresponding index for specified string in combobox
                ...
                wdgt_name = obj.objectName()
                try:
                    value = settings.value('/'.join([prefix,
                                                     wdgt_name]))  # get stored value from registry
...
def property_change_handler(value_name, conversion=lambda x: x, setter_name=None, getter_name=None):
    """Generate a function that produces callback functions.
...
auto_connectable_controls['checkbox'] = {
    ...
    'control_change_handler':
        control_change_handler(lambda x: x == QtCore.Qt.Checked),
    'control_change_slot_name':
        'stateChanged',
    'property_change_handler':
        property_change_handler("checkState", lambda x: QtCore.Qt.Checked
                                if x else QtCore.Qt.Unchecked),}
...
auto_connectable_controls['doublespinbox'] = {
    ...
    'suffices': [
        "_doubleSpinBox", "_spinbox", "SpinBox", "_spin", "_doublespinbox", "DoubleSpinBox"],
    'control_change_handler': control_change_handler(),
    'control_change_slot_name': 'valueChanged',
